...
# C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_EIF4A3_K562 --> working directory of the project
# SRR3469574 --> Sample ID of first WT sample
# SRR3469575 --> Sample ID of second WT sample
# SRR3469594 --> Sample ID of first KD sample
# SRR3469595 --> Sample ID of second KD sample
# EIF4A3 --> Ensemble name of your gene of interest
...
samples <- read_excel("ENCODE_EIF4A3_K562_samples.xlsx")
View(samples)


#Load in the files
files1 <- file.path("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_EIF4A3_K562","koutput", samples$Run, "abundance.h5")
files1
names(files1) <- paste0(c("SRR3469574","SRR3469575","SRR3469594","SRR3469595"))
...
counts$mean=rowMeans(counts[,c("SRR3469574","SRR3469575")], na.rm=TRUE) #Filter WT samples
counts=filter(counts, mean>=1)
counts$mean=rowMeans(counts[,c("SRR3469594","SRR3469595")], na.rm=TRUE) #Fiter KD samples
counts=filter(counts, mean>=1)
counts$mean <- NULL
...
EIF4A3transcripts=filter(listgenes, external_gene_name == "EIF4A3" )
EIF4A3transcriptsWT<-EIF4A3transcripts %>% dplyr::select(SRR3469574,SRR3469575, ensembl_transcript_id_version, external_gene_name, external_transcript_name)
EIF4A3transcriptsWT$averagecounts = rowMeans(EIF4A3transcriptsWT[,c("SRR3469574","SRR3469575")])
#put in specific library names above
EIF4A3transcriptsWT$percent= EIF4A3transcriptsWT$averagecounts/sum(EIF4A3transcriptsWT$averagecounts)
EIF4A3transcriptsWT<-EIF4A3transcriptsWT %>% arrange(averagecounts)

#Repeat for the KD transcripts
EIF4A3transcriptsKD<-EIF4A3transcripts %>% dplyr::select(SRR3469594,SRR3469595, ensembl_transcript_id_version, external_gene_name, external_transcript_name)
EIF4A3transcriptsKD$averagecounts = rowMeans(EIF4A3transcriptsKD[,c("SRR3469594","SRR3469595")])
EIF4A3transcriptsKD$percent= EIF4A3transcriptsKD$averagecounts/sum(EIF4A3transcriptsKD$averagecounts)
EIF4A3transcriptsKD<-EIF4A3transcriptsKD %>% arrange(averagecounts)
...
plot(euler(s), quantities = TRUE,
     main="EIF4A3 knockout vs control")
...
NMDBio_cdf = ggplot(alltransbioNMD, aes(log2FoldChange, colour=transcript_biotype))+
  ...
  labs(y = "Cumulative Frequency", title = "EIF4A3 KD vs WT", subtitle = "NMD biotype")
NMDBio_cdf

#analysis with robert's PTC+ and PTC- list
ENST_PTC.EPI.TFG <- read.delim("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_EIF4A3_K562/ENST_PTC-EPI-TFG.txt")
...
PTC_cdf = ggplot(alltransPTC, aes(log2FoldChange, colour=PTC.Status))+
  ...
  coord_cartesian(xlim = c(-3,3)) +
  scale_color_manual(values = met.brewer("Java",2)) +
  labs(y = "Cumulative Frequency", title = "EIF4A3 KD vs WT", subtitle = "PTC list")
PTC_cdf
...
NMD_transcripts = read.delim("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_EIF4A3_K562/NMD_transcript_list.txt")
...
NMD_cdf = ggplot(alltransNMD, aes(log2FoldChange, colour=NMD))+
  ...
  coord_cartesian(xlim = c(-3,3)) +
  scale_color_manual(values = met.brewer("Java",2)) +
  labs(y = "Cumulative Frequency", title = "EIF4A3 KD vs WT", subtitle = "NMD +/- list")
NMD_cdf
...
StringentNMD = read.csv("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_EIF4A3_K562/Stringent_NMD_list_CE")
...
sNMD_cdf = ggplot(alltransSTNMD, aes(log2FoldChange, colour=NMD))+
  ...
  coord_cartesian(xlim = c(-3,3)) +
  scale_color_manual(values = met.brewer("Java",2)) +
  labs(y = "Cumulative Frequency", title = "EIF4A3 KD vs WT", subtitle = "Stringent NMD list")
sNMD_cdf
...
sPTC = read.csv("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_EIF4A3_K562/Stringent_PTC_CE") #
alltransSPTC = inner_join(alltrans, sPTC, by = c("ENST.ID" = "transID")) 
sPTC_res <- wilcox.test(log2FoldChange ~ PTC, data = alltransSPTC,
                        exact = FALSE, alternative = "less")
count(alltransSPTC, PTC) #714 PTC false 268 PTC true
head(alltransSPTC)
sPTC_res #p-vlaue = 2.2e-16
...
sPTC_cdf = sPTC_cdf + annotate("text", x=-2.5,y=1, label = "EIF4A3 KD vs WT", size = 5, hjust = 0) +
  annotate("text", x=-2.5,y=.95, label = "sPTC containing transcripts", hjust =0) +
  annotate("text", x=-2.5,y=.9, label = "p-value = 2.2e-16", hjust = 0) +
  annotate("text", x = -2.5, y = 0.85, label = "PTC+ = 268", hjust = 0) +
  annotate("text", x = -2.5, y = 0.8, label = "PTC- = 714", hjust = 0) +
  geom_vline(xintercept = 0, color = "grey") + 
  geom_hline(yintercept = 0.5, color = "grey") +
  theme(legend.position = c(0.85,0.1))
sPTC_cdf
ggsave("EIF4A3_K562_sPTC_CDF.pdf", 
       plot = sPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
       ...
       device = "pdf",
       dpi = 300)
ggsave("C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/EIF4A3_K562_sPTC_CDF.pdf", 
       plot = sPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
...
EIF4A3_x = alltransSTNMD %>% filter(NMD == "TRUE")  %>% #Makes a data table of the top NMD transcript for each gene
  select(Gene,log2FoldChange,baseMean) %>% 
  rename(plusFC = log2FoldChange) %>% 
  group_by(Gene) %>% 
  slice_max(baseMean, n = 1)

EIF4A3_y  = alltransSTNMD %>% filter(NMD == "FALSE")  %>% #Makes a data table fo the top NMD- transcript for each gene
  select(Gene,log2FoldChange,baseMean) %>% 
  rename(minusFC = log2FoldChange) %>% 
  group_by(Gene) %>% 
  slice_max(baseMean, n = 1)

EIF4A3_scatter = full_join(EIF4A3_x, EIF4A3_y, by = "Gene", suffix = c("plus","minus")) #Combine the two top tables
EIF4A3_scatter = EIF4A3_scatter %>% filter(!is.na(minusFC)) %>% 
  filter(!is.na(plusFC)) #Removes any NA rows


dot = ggplot(data = EIF4A3_scatter)+
  ...
  labs(x = "Log2FC NMD+", y = "Log2FC NMD-", title = "EIF4A3 gene-level FC comparison")+
  theme_minimal()+
...
ASgenes_EIF4A3 <- read_delim("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/rMATS/ENCODE_EIF4A3/ASgenes_EIF4A3.csv", 
                          delim = ";", escape_double = FALSE, trim_ws = TRUE)
ASgenes_EIF4A3 = ASgenes_EIF4A3 %>% select(GeneID,geneSymbol)
...
sPTC_AS = sPTC_gene %>% anti_join(ASgenes_EIF4A3, by = c("ensembl_gene_id" = "GeneID"))
...
asPTC_cdf = ggplot(alltransAS, aes(log2FoldChange, colour=PTC))+
  ...
  coord_cartesian(xlim = c(-3,3)) +
  scale_color_manual(values = met.brewer("Java",2)) +
  labs(y = "Cumulative Frequency", title = "EIF4A3 KD vs WT", subtitle = "Non-AS transcripts")
asPTC_cdf = asPTC_cdf + annotate("text", x=-2.5,y=1, label = "p-value = ") +
  ...
  geom_hline(yintercept = 0.5, color = "grey")
asPTC_cdf
ggsave("EIF4A3_K562_asPTC_CDF.pdf", 
       plot = asPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
...
EIF4A3_NMD = alltrans_MANE %>% inner_join(NMD_factors, by = c("external_gene_name" = "NMDfactor"))
write.csv(EIF4A3_NMD, "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/NMD_analysis/EIF4A3_NMDfactor.csv",row.names = FALSE)
...
count(shared_SPTC_trans, PTC)#175 PTC false 87 PTC true
shared_sPTC_res #p-vlaue = 9.245e-10
...
shared_sPTC_cdf = ggplot(shared_SPTC_trans, aes(log2FoldChange, colour=PTC))+
  ...
  coord_cartesian(xlim = c(-3,3)) +
  scale_color_manual(values = plot_colors) +
  labs(y = "Cumulative Frequency", title = "EIF4A3 KD vs WT", subtitle = "Shared stringent PTC transcripts")
shared_sPTC_cdf = shared_sPTC_cdf + annotate("text", x=-2.5,y=1, label = "p-value = 9.245e-10") +
  annotate("text", x = -2.5, y = 0.95, label = "PTC+ = 87") +
  annotate("text", x = -2.5, y = 0.9, label = "PTC- = 175") +
  geom_vline(xintercept = 0, color = "grey") + 
  geom_hline(yintercept = 0.5, color = "grey")
shared_sPTC_cdf
ggsave("EIF4A3_K562_shared_PTC_CDF.pdf", 
...
PE_res <- wilcox.test(log2FoldChange ~ isoform, data = PE_FC,
                               exact = FALSE, alternative = "less")
head(PE_FC)
count(PE_FC, isoform)# 54 PE  64 MANE
PE_res #p-vlaue = 0.0942
...
PE_cdf = ggplot(PE_FC, aes(log2FoldChange, colour=isoform))+
  ...
  scale_color_manual(values = plot_colors) +
  labs(y = "Cumulative Frequency")
PE_cdf = PE_cdf + annotate("text", x=-2.5,y=1, label = "EIF4A3 KD vs WT", size = 5, hjust = 0) +
  annotate("text", x=-2.5,y=.95, label = "Transcripts with a poison exon", hjust =0) +
  annotate("text", x=-2.5,y=.9, label = "p-value = 0.0942", hjust = 0) +
  annotate("text", x = -2.5, y = 0.85, label = "PE = 54", hjust = 0) +
  annotate("text", x = -2.5, y = 0.8, label = "MANE = 64", hjust = 0) +
  geom_vline(xintercept = 0, color = "grey") + 
  geom_hline(yintercept = 0.5, color = "grey") +
  theme(legend.position = c(0.85,0.1))
PE_cdf
ggsave("EIF4A3_K562_PE_CDF.pdf", 
       plot = PE_cdf,
       scale = 1,
       width = 8,
       height = 6,
       ...
       device = "pdf",
       dpi = 300)
ggsave("C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/EIF4A3_K562_PE_CDF.pdf", 
       plot = PE_cdf,
       scale = 1,
       width = 8,
       height = 6,
...
PEa_res <- wilcox.test(log2FoldChange ~ isoform, data = PEa_FC,
                      exact = FALSE, alternative = "less")
head(PEa_FC)
count(PEa_FC, isoform)# 284 PE  208 MANE
PEa_res #p-vlaue = 0.2631
...
PEa_cdf = ggplot(PEa_FC, aes(log2FoldChange, colour=isoform))+
  ...
  scale_color_manual(values = plot_colors) +
  labs(y = "Cumulative Frequency")
PEa_cdf = PEa_cdf + annotate("text", x=-2.5,y=1, label = "EIF4A3 KD vs WT", size = 5, hjust = 0) +
  annotate("text", x=-2.5,y=.95, label = "Transcripts with a poison exon", hjust =0) +
  annotate("text", x=-2.5,y=.9, label = "p-value = 0.2631", hjust = 0) +
  annotate("text", x = -2.5, y = 0.85, label = "PE = 284", hjust = 0) +
  annotate("text", x = -2.5, y = 0.8, label = "MANE = 208", hjust = 0) +
  geom_vline(xintercept = 0, color = "grey") + 
  geom_hline(yintercept = 0.5, color = "grey") +
  theme(legend.position = c(0.85,0.1))
PEa_cdf
ggsave("EIF4A3_K562_PE_archive_CDF.pdf", 
       plot = PEa_cdf,
       scale = 1,
       width = 8,
       height = 6,
       ...
       device = "pdf",
       dpi = 300)
ggsave("C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/EIF4A3_K562_PEarchive_CDF.pdf", 
       plot = PEa_cdf,
       scale = 1,
       width = 8,
       height = 6,
...
EIF4A3_SC_impact = alltrans_MANE %>% inner_join(Splicing_factors, by = c("external_gene_name" = "Component"))
write.csv(EIF4A3_SC_impact, "EIF4A3_splicing_impact.csv",row.names = FALSE)

##Redo the sPTC analysis using the MANE list
sPTC_MANE = read.csv("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_EIF4A3_K562/Stringent_PTC_MANE_CE.csv")
alltransSPTC_MANE = inner_join(alltrans, sPTC_MANE, by = c("ENST.ID" = "transID"))
write_csv(alltransSPTC_MANE,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/EIF4A3_PTC_alltrans.csv")
sPTC_MANE_res <- wilcox.test(log2FoldChange ~ PTC, data = alltransSPTC_MANE,
                        exact = FALSE, alternative = "less")
count(alltransSPTC_MANE, PTC) #235 PTC false 268 PTC true
head(alltransSPTC_MANE)
sPTC_MANE_res #p-vlaue = <2.2e-16
...
MANE_sPTC_cdf = ggplot(alltransSPTC_MANE, aes(log2FoldChange, colour=PTC))+
  ...
  scale_color_manual(values = plot_colors) +
  labs(y = "Cumulative Frequency")
MANE_sPTC_cdf = MANE_sPTC_cdf + annotate("text", x=-2.5,y=1, label = "EIF4A3 KD vs WT", size = 5, hjust = 0) +
  annotate("text", x=-2.5,y=.95, label = "PTC containing and MANE transcripts", hjust =0) +
  annotate("text", x=-2.5,y=.9, label = "p-value = 2.2e-16", hjust = 0) +
  annotate("text", x = -2.5, y = 0.85, label = "PTC+ = 268", hjust = 0) +
  annotate("text", x = -2.5, y = 0.8, label = "PTC- = 235", hjust = 0) +
  geom_vline(xintercept = 0, color = "grey") + 
  geom_hline(yintercept = 0.5, color = "grey") +
  theme(legend.position = c(0.85,0.1))
MANE_sPTC_cdf
ggsave("EIF4A3_K562_sPTC_MANE_CDF.pdf", 
       plot = MANE_sPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
       ...
       device = "pdf",
       dpi = 300)
ggsave("C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/EIF4A3_K562_sPTC_MANE_CDF.pdf", 
       plot = MANE_sPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
...
write_csv(PTC_tpm, "EIF4A3_PTC_MANE_TPM.csv")
write_csv(PTC_tpm, "C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/EIF4A3_PTC_MANE_TPM.csv")

#### Look at the effect when only WT filtering####
WTcounts <-as.data.frame(txi.kallisto.tsv$counts)

#Filtering out transcripts with mean tpm<1
WTcounts$mean=rowMeans(WTcounts[,c("SRR3469574","SRR3469575")], na.rm=TRUE) #Filter WT samples
...
write_csv(WTalltrans,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/EIF4A3_WTfilt_alltrans.csv")
write_csv(alltrans,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/EIF4A3_DBfilt_alltrans.csv")
write_csv(alltransSPTC_MANE,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/EIF4A3_PTC_alltrans.csv")
...
WT5counts$mean=rowMeans(WT5counts[,c("SRR3469574","SRR3469575")], na.rm=TRUE) #Filter WT samples
...
write_csv(WT5alltrans,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/EIF4A3_WT5filt_alltrans.csv")
