...
#C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_MAGOH_K562 --> working directory of the project
...
setwd(dir = "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_MAGOH_K562")

#Load File containing sample types

samples <- read_excel("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_MAGOH_K562/samples.xlsx")

View(samples)

#Load in the files

files1 <- file.path("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_MAGOH_K562","koutput", samples$Run, "abundance.h5")
...
MAGOH_K562transcripts=filter(listgenes, external_gene_name == "MAGOH" )

MAGOH_K562transcriptsWT<-MAGOH_K562transcripts %>% dplyr::select(WT1,WT2, ensembl_transcript_id_version, external_gene_name, external_transcript_name)

MAGOH_K562transcriptsWT$averagecounts = rowMeans(MAGOH_K562transcriptsWT[,c("WT1","WT2")])

#put in specific library names above

MAGOH_K562transcriptsWT$percent= MAGOH_K562transcriptsWT$averagecounts/sum(MAGOH_K562transcriptsWT$averagecounts)

MAGOH_K562transcriptsWT<-MAGOH_K562transcriptsWT %>% arrange(averagecounts)

#Repeat for the KD transcripts

MAGOH_K562transcriptsKD<-MAGOH_K562transcripts %>% dplyr::select(KD1,KD2, ensembl_transcript_id_version, external_gene_name, external_transcript_name)

MAGOH_K562transcriptsKD$averagecounts = rowMeans(MAGOH_K562transcriptsKD[,c("KD1","KD2")])

MAGOH_K562transcriptsKD$percent= MAGOH_K562transcriptsKD$averagecounts/sum(MAGOH_K562transcriptsKD$averagecounts)

MAGOH_K562transcriptsKD<-MAGOH_K562transcriptsKD %>% arrange(averagecounts)
...
plot(euler(s), quantities = TRUE,
     main="MAGOH knockout vs control")
...
NMDBio_cdf = ggplot(alltransbioNMD, aes(log2FoldChange, colour=transcript_biotype))+
  ...
  labs(y = "Cumulative Frequency", title = "MAGOH KD vs WT", subtitle = "NMD biotype")
NMDBio_cdf

#### analysis with robert's PTC+ and PTC- list ####
ENST_PTC.EPI.TFG <- read.delim("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_MAGOH_K562/ENST_PTC-EPI-TFG.txt")
...
PTC_cdf = ggplot(alltransPTC, aes(log2FoldChange, colour=PTC.Status))+
  ...
  coord_cartesian(xlim = c(-3,3)) +
  scale_color_manual(values = met.brewer("Java",2)) +
  labs(y = "Cumulative Frequency", title = "MAGOH KD vs WT", subtitle = "PTC list")
PTC_cdf
...
NMD_transcripts = read.delim("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_MAGOH_K562/NMD_transcript_list.txt")
...
NMD_cdf = ggplot(alltransNMD, aes(log2FoldChange, colour=NMD))+
  ...
  coord_cartesian(xlim = c(-3,3)) +
  scale_color_manual(values = met.brewer("Java",2)) +
  labs(y = "Cumulative Frequency", title = "MAGOH KD vs WT", subtitle = "NMD +/- list")
NMD_cdf
...
StringentNMD = read.csv("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_MAGOH_K562/Stringent_NMD_list_CE")
...
sNMD_cdf = ggplot(alltransSTNMD, aes(log2FoldChange, colour=NMD))+
  ...
  coord_cartesian(xlim = c(-3,3)) +
  scale_color_manual(values = met.brewer("Java",2)) +
  labs(y = "Cumulative Frequency", title = "MAGOH KD vs WT", subtitle = "Stringent NMD list")
sNMD_cdf
...
sPTC = read.csv("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_MAGOH_K562/Stringent_PTC_CE") 
alltransSPTC = inner_join(alltrans, sPTC, by = c("ENST.ID" = "transID")) 
sPTC_res <- wilcox.test(log2FoldChange ~ PTC, data = alltransSPTC,
                        exact = FALSE, alternative = "less")
head(alltransSPTC)
count(alltransSPTC, PTC) #818 PTC false 280 PTC true
sPTC_res #p-vlaue = 2.341e-8
...
sPTC_cdf = sPTC_cdf + annotate("text", x=-2.5,y=1, label = "MAGOH KD vs WT", size = 5, hjust = 0) +
  annotate("text", x=-2.5,y=.95, label = "Stringent PTC containing transcripts", hjust =0) +
  annotate("text", x=-2.5,y=.9, label = "p-value = 2.341e-8", hjust = 0) +
  annotate("text", x = -2.5, y = 0.85, label = "PTC+ = 280", hjust = 0) +
  annotate("text", x = -2.5, y = 0.8, label = "PTC- = 818", hjust = 0) +
  geom_vline(xintercept = 0, color = "grey") + 
  geom_hline(yintercept = 0.5, color = "grey") +
  theme(legend.position = c(0.85,0.1))
sPTC_cdf
ggsave("MAGOH_K562_sPTC_CDF.pdf", 
       plot = sPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
       ...
       device = "pdf",
       dpi = 300)
ggsave("C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/MAGOH_K562_sPTC_CDF.pdf", 
       plot = sPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
...
MAGOH_x = alltransSTNMD %>% filter(NMD == "TRUE")  %>% #Makes a data table of the top NMD transcript for each gene
  select(Gene,log2FoldChange,baseMean) %>% 
  rename(plusFC = log2FoldChange) %>% 
  group_by(Gene) %>% 
  slice_max(baseMean, n = 1)

MAGOH_y  = alltransSTNMD %>% filter(NMD == "FALSE")  %>% #Makes a data table fo the top NMD- transcript for each gene
  select(Gene,log2FoldChange,baseMean) %>% 
  rename(minusFC = log2FoldChange) %>% 
  group_by(Gene) %>% 
  slice_max(baseMean, n = 1)

MAGOH_scatter = full_join(MAGOH_x, MAGOH_y, by = "Gene", suffix = c("plus","minus")) #Combine the two top tables
MAGOH_scatter = MAGOH_scatter %>% filter(!is.na(minusFC)) %>% 
  filter(!is.na(plusFC)) #Removes any NA rows


dot = ggplot(data = MAGOH_scatter)+
  ...
  labs(x = "Log2FC NMD+", y = "Log2FC NMD-", title = "MAGOH gene-level FC comparison")+
  theme_minimal()+
...
MAGOH_NMD = alltrans_MANE %>% inner_join(NMD_factors, by = c("external_gene_name" = "NMDfactor"))
write.csv(MAGOH_NMD, "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/NMD_analysis/MAGOH_NMDfactor.csv",row.names = FALSE)
...
sPTC_MANE_res <- wilcox.test(log2FoldChange ~ PTC, data = alltransSPTC_MANE,
                             exact = FALSE, alternative = "less")
count(alltransSPTC_MANE, PTC) #262 PTC false 280 PTC true
sPTC_MANE_res #p-vlaue = 5.5e-8
...
MANE_sPTC_cdf = ggplot(alltransSPTC_MANE, aes(log2FoldChange, colour=PTC))+
  ...
  scale_color_manual(values = plot_colors) +
  labs(y = "Cumulative Frequency")
MANE_sPTC_cdf = MANE_sPTC_cdf + annotate("text", x=-2.5,y=1, label = "MAGOH KD vs WT", size = 5, hjust = 0) +
  annotate("text", x=-2.5,y=.95, label = "PTC containing and MANE transcripts", hjust =0) +
  annotate("text", x=-2.5,y=.9, label = "p-value = 5.5e-8", hjust = 0) +
  annotate("text", x = -2.5, y = 0.85, label = "PTC+ = 280", hjust = 0) +
  ...
  theme(legend.position = c(0.85,0.1))
MANE_sPTC_cdf
ggsave("MAGOH_K562_sPTC_MANE_CDF.pdf", 
       plot = MANE_sPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
       ...
       device = "pdf",
       dpi = 300)
ggsave("C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/MAGOH_K562_sPTC_MANE_CDF.pdf", 
       plot = MANE_sPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
...
PEa_res <- wilcox.test(log2FoldChange ~ isoform, data = PEa_FC,
                       exact = FALSE, alternative = "less")
head(PEa_FC)
count(PEa_FC, isoform)# 319 PE  222 MANE
PEa_res #p-vlaue = 0.999
...
PEa_cdf = ggplot(PEa_FC, aes(log2FoldChange, colour=isoform))+
  ...
  scale_color_manual(values = plot_colors) +
  labs(y = "Cumulative Frequency")
PEa_cdf = PEa_cdf + annotate("text", x=-2.5,y=1, label = "MAGOH KD vs WT", size = 5, hjust = 0) +
  annotate("text", x=-2.5,y=.95, label = "Transcripts with a poison exon", hjust =0) +
  annotate("text", x=-2.5,y=.9, label = "p-value = 0.999", hjust = 0) +
  annotate("text", x = -2.5, y = 0.85, label = "PE = 319", hjust = 0) +
  annotate("text", x = -2.5, y = 0.8, label = "MANE = 222", hjust = 0) +
  geom_vline(xintercept = 0, color = "grey") + 
  geom_hline(yintercept = 0.5, color = "grey") +
  theme(legend.position = c(0.85,0.1))
PEa_cdf
ggsave("MAGOH_K562_PE_archive_CDF.pdf", 
       plot = PEa_cdf,
       scale = 1,
       width = 8,
       height = 6,
       ...
       device = "pdf",
       dpi = 300)
ggsave("C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/MAGOH_K562_PEarchive_CDF.pdf", 
       plot = PEa_cdf,
       scale = 1,
       width = 8,
       height = 6,
...
MAGOH_SC_impact = alltrans_MANE %>% inner_join(Splicing_factors, by = c("external_gene_name" = "Component"))
write.csv(MAGOH_SC_impact, "MAGOH_splicing_impact.csv",row.names = FALSE)
...
write_csv(PTC_tpm, "MAGOH_PTC_MANE_TPM.csv")
write_csv(PTC_tpm, "C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/MAGOH_PTC_MANE_TPM.csv")
...
write_csv(WT5alltrans,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/MAGOH_WT5filt_alltrans.csv")
...
write_csv(WTalltrans,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/MAGOH_WTfilt_alltrans.csv")
write_csv(alltrans,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/MAGOH_DBfilt_alltrans.csv")
write_csv(alltransSPTC_MANE,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/MAGOH_PTC_alltrans.csv")
