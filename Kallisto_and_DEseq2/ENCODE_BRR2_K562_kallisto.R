...
# C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_BRR2_K562 --> working directory of the project
# SRR4421417 --> Sample ID of first WT sample
# SRR4421418 --> Sample ID of second WT sample
# SRR4422525 --> Sample ID of first KD sample
# SRR4422526 --> Sample ID of second KD sample
# SNRNP200 --> Ensemble name of your gene of interest
...
#Load in the files
files1 <- file.path("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_BRR2_K562","koutput", samples$Run, "abundance.h5")
files1
names(files1) <- paste0(c("SRR4421417","SRR4421418","SRR4422525","SRR4422526"))
...
counts$mean=rowMeans(counts[,c("SRR4421417","SRR4421418")], na.rm=TRUE) #Filter WT samples
counts=filter(counts, mean>=1)
counts$mean=rowMeans(counts[,c("SRR4422525","SRR4422526")], na.rm=TRUE) #Fiter KD samples
counts=filter(counts, mean>=1)
counts$mean <- NULL
...
SNRNP200transcripts=filter(listgenes, external_gene_name == "SNRNP200" )
SNRNP200transcriptsWT<-SNRNP200transcripts %>% dplyr::select(SRR4421417,SRR4421418, ensembl_transcript_id_version, external_gene_name, external_transcript_name)
SNRNP200transcriptsWT$averagecounts = rowMeans(SNRNP200transcriptsWT[,c("SRR4421417","SRR4421418")])
#put in specific library names above
SNRNP200transcriptsWT$percent= SNRNP200transcriptsWT$averagecounts/sum(SNRNP200transcriptsWT$averagecounts)
SNRNP200transcriptsWT<-SNRNP200transcriptsWT %>% arrange(averagecounts)

#Repeat for the KD transcripts
SNRNP200transcriptsKD<-SNRNP200transcripts %>% dplyr::select(SRR4422525,SRR4422526, ensembl_transcript_id_version, external_gene_name, external_transcript_name)
SNRNP200transcriptsKD$averagecounts = rowMeans(SNRNP200transcriptsKD[,c("SRR4422525","SRR4422526")])
SNRNP200transcriptsKD$percent= SNRNP200transcriptsKD$averagecounts/sum(SNRNP200transcriptsKD$averagecounts)
SNRNP200transcriptsKD<-SNRNP200transcriptsKD %>% arrange(averagecounts)
...
plot(euler(s), quantities = TRUE,
     main="SNRNP200 knockout vs control")
...
NMDBio_cdf = ggplot(alltransbioNMD, aes(log2FoldChange, colour=transcript_biotype))+
  ...
  scale_color_manual(values = met.brewer("Java", 23)) +
  labs(y = "Cumulative Frequency", title = "SNRNP200 KD vs WT", subtitle = "NMD biotype")
NMDBio_cdf

#analysis with robert's PTC+ and PTC- list
ENST_PTC.EPI.TFG <- read.delim("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_BRR2_K562/ENST_PTC-EPI-TFG.txt")
...
PTC_cdf = ggplot(alltransPTC, aes(log2FoldChange, colour=PTC.Status))+
  ...
  labs(y = "Cumulative Frequency", title = "SNRNP200 KD vs WT", subtitle = "PTC list")
PTC_cdf
...
NMD_transcripts = read.delim("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_BRR2_K562/NMD_transcript_list.txt")
...
NMD_cdf = ggplot(alltransNMD, aes(log2FoldChange, colour=NMD))+
  ...
  coord_cartesian(xlim = c(-3,3)) +
  scale_color_manual(values = met.brewer("Java",2)) +
  labs(y = "Cumulative Frequency", title = "SNRNP200 KD vs WT", subtitle = "NMD +/- list")
NMD_cdf
...
StringentNMD = read.csv("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_BRR2_K562/Stringent_NMD_list_CE")
...
sNMD_cdf = ggplot(alltransSTNMD, aes(log2FoldChange, colour=NMD))+
  ...
  coord_cartesian(xlim = c(-3,3)) +
  scale_color_manual(values = met.brewer("Java",2)) +
  labs(y = "Cumulative Frequency", title = "SNRNP200 KD vs WT", subtitle = "Stringent NMD list")
sNMD_cdf
...
sPTC = read.csv("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_BRR2_K562/Stringent_PTC_CE") #
alltransSPTC = inner_join(alltrans, sPTC, by = c("ENST.ID" = "transID")) #906 PTC false 278 PTC true
sPTC_res <- wilcox.test(log2FoldChange ~ PTC, data = alltransSPTC,
                        exact = FALSE, alternative = "less")

alltransSPTC %>% group_by(PTC) %>% summarise(count = n())
head(alltransSPTC)
sPTC_res #p-vlaue = 0.01982
...
sPTC_cdf = sPTC_cdf + annotate("text", x=-2.5,y=1, label = "SNRNP200 KD vs WT", size = 5, hjust = 0) +
  annotate("text", x=-2.5,y=.95, label = "Stringent PTC containing transcripts", hjust =0) +
  annotate("text", x=-2.5,y=.9, label = "p-value = 0.01982", hjust = 0) +
  annotate("text", x = -2.5, y = 0.85, label = "PTC+ = 278", hjust = 0) +
  annotate("text", x = -2.5, y = 0.8, label = "PTC- = 906", hjust = 0) +
  geom_vline(xintercept = 0, color = "grey") + 
  geom_hline(yintercept = 0.5, color = "grey") +
  theme(legend.position = c(0.85,0.1))
sPTC_cdf
ggsave("SNRNP200_K562_sPTC_CDF.pdf", 
       plot = sPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
       ...
       device = "pdf",
       dpi = 300)
ggsave("C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/SNRNP200_K562_sPTC_CDF.pdf", 
       plot = sPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
...
SNRNP200_x = alltransSTNMD %>% filter(NMD == "TRUE")  %>% #Makes a data table of the top NMD transcript for each gene
  select(Gene,log2FoldChange,baseMean) %>% 
  rename(plusFC = log2FoldChange) %>% 
  group_by(Gene) %>% 
  slice_max(baseMean, n = 1)

SNRNP200_y  = alltransSTNMD %>% filter(NMD == "FALSE")  %>% #Makes a data table fo the top NMD- transcript for each gene
  select(Gene,log2FoldChange,baseMean) %>% 
  rename(minusFC = log2FoldChange) %>% 
  group_by(Gene) %>% 
  slice_max(baseMean, n = 1)

SNRNP200_scatter = full_join(SNRNP200_x, SNRNP200_y, by = "Gene", suffix = c("plus","minus")) #Combine the two top tables
SNRNP200_scatter = SNRNP200_scatter %>% filter(!is.na(minusFC)) %>% 
  filter(!is.na(plusFC)) #Removes any NA rows


dot = ggplot(data = SNRNP200_scatter)+
  ...
  labs(x = "Log2FC NMD+", y = "Log2FC NMD-", title = "SNRNP200 gene-level FC comparison")+
  theme_minimal()+
...
#Check the effect on NMD targets not alternately spliced
ASgenes_SNRNP200 <- read_delim("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/rMATS/ENCODE_SNRNP200/ASgenes_SNRNP200.csv", 
                          delim = ";", escape_double = FALSE, trim_ws = TRUE)
ASgenes_SNRNP200 = ASgenes_SNRNP200 %>% select(GeneID,geneSymbol)
...
sPTC_AS = sPTC_gene %>% anti_join(ASgenes_SNRNP200, by = c("ensembl_gene_id" = "GeneID"))
count(sPTC_AS, PTC) #PTC false= 385 PTC true= 99
alltransAS = inner_join(alltrans, sPTC_AS, by = c("ENST.ID" = "ensembl_transcript_id"))
count(alltransAS, PTC) # 128 PTC false  34 PTC true


asPTC_res <- wilcox.test(log2FoldChange ~ PTC, data = alltransAS,
                         exact = FALSE, alternative = "less")
head(alltransAS)
asPTC_res #p-vlaue = 0.9162
...
asPTC_cdf = ggplot(alltransAS, aes(log2FoldChange, colour=PTC))+
  ...
  coord_cartesian(xlim = c(-3,3)) +
  scale_color_manual(values = met.brewer("Java",2)) +
  labs(y = "Cumulative Frequency", title = "SNRNP200 KD vs WT", subtitle = "Non-AS transcripts")
asPTC_cdf = asPTC_cdf + annotate("text", x=-2.5,y=1, label = "p-value = 0.9162") +
  annotate("text", x = -2.5, y = 0.95, label = "PTC+ = 34") +
  annotate("text", x = -2.5, y = 0.9, label = "PTC- = 128") +
  geom_vline(xintercept = 0, color = "grey") + 
  geom_hline(yintercept = 0.5, color = "grey")
asPTC_cdf
ggsave("SNRNP200_K562_asPTC_CDF.pdf", 
       plot = asPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
...
SNRNP200_NMD = alltrans_MANE %>% inner_join(NMD_factors, by = c("external_gene_name" = "NMDfactor"))
write.csv(SNRNP200_NMD, "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/NMD_analysis/SNRNP200_NMDfactor.csv",row.names = FALSE)
...
PE_res <- wilcox.test(log2FoldChange ~ isoform, data = PE_FC,
                      exact = FALSE, alternative = "less")
head(PE_FC)
count(PE_FC, isoform)# 71 PE  69 MANE
PE_res #p-vlaue = 0.05952
...
PE_cdf = ggplot(PE_FC, aes(log2FoldChange, colour=isoform))+
  ...
  labs(y = "Cumulative Frequency")
PE_cdf = PE_cdf + annotate("text", x=-2.5,y=1, label = "SNRNP200 KD vs WT", size = 5, hjust = 0) +
  annotate("text", x=-2.5,y=.95, label = "Transcripts with a poison exon", hjust =0) +
  annotate("text", x=-2.5,y=.9, label = "p-value = 0.05952", hjust = 0) +
  annotate("text", x = -2.5, y = 0.85, label = "PE = 71", hjust = 0) +
  annotate("text", x = -2.5, y = 0.8, label = "MANE = 69", hjust = 0) +
  geom_vline(xintercept = 0, color = "grey") + 
  geom_hline(yintercept = 0.5, color = "grey") +
  theme(legend.position = c(0.85,0.1))
PE_cdf
ggsave("SNRNP200_K562_PE_CDF.pdf", 
       plot = PE_cdf,
       scale = 1,
       width = 8,
       height = 6,
       ...
       device = "pdf",
       dpi = 300)
ggsave("C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/SNRNP200_K562_PE_CDF.pdf", 
       plot = PE_cdf,
       scale = 1,
       width = 8,
       height = 6,
...
PEa_res <- wilcox.test(log2FoldChange ~ isoform, data = PEa_FC,
                       exact = FALSE, alternative = "less")
head(PEa_FC)
count(PEa_FC, isoform)# 350 PE 226  MANE
PEa_res #p-vlaue = 2e-11
...
PEa_cdf = ggplot(PEa_FC, aes(log2FoldChange, colour=isoform))+
  ...
  scale_color_manual(values = plot_colors) +
  labs(y = "Cumulative Frequency")
PEa_cdf = PEa_cdf + annotate("text", x=-2.5,y=1, label = "SNRNP200 KD vs WT", size = 5, hjust = 0) +
  annotate("text", x=-2.5,y=.95, label = "Transcripts with a poison exon", hjust =0) +
  annotate("text", x=-2.5,y=.9, label = "p-value = 2e-11", hjust = 0) +
  annotate("text", x = -2.5, y = 0.85, label = "PE = 350", hjust = 0) +
  annotate("text", x = -2.5, y = 0.8, label = "MANE = 226", hjust = 0) +
  geom_vline(xintercept = 0, color = "grey") + 
  geom_hline(yintercept = 0.5, color = "grey") +
  theme(legend.position = c(0.85,0.1))
PEa_cdf
ggsave("SNRNP200_K562_PE_archive_CDF.pdf", 
       plot = PEa_cdf,
       scale = 1,
       width = 8,
       height = 6,
       ...
       device = "pdf",
       dpi = 300)
ggsave("C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/SNRNP200_K562_PEarchive_CDF.pdf", 
       plot = PEa_cdf,
       scale = 1,
       width = 8,
       height = 6,
...
sPTC_MANE = read.csv("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_BRR2_K562/Stringent_PTC_MANE_CE.csv")
...
sPTC_MANE_res <- wilcox.test(log2FoldChange ~ PTC, data = alltransSPTC_MANE,
                             exact = FALSE, alternative = "less")
count(alltransSPTC_MANE, PTC) # 253 PTC false 278 PTC true
sPTC_MANE_res #p-vlaue = 2.322e-3
...
MANE_sPTC_cdf = ggplot(alltransSPTC_MANE, aes(log2FoldChange, colour=PTC))+
  ...
  scale_color_manual(values = plot_colors) +
  labs(y = "Cumulative Frequency")
MANE_sPTC_cdf = MANE_sPTC_cdf + annotate("text", x=-2.5,y=1, label = "SNRNP200 KD vs WT", size = 5, hjust = 0) +
  annotate("text", x=-2.5,y=.95, label = "PTC containing and MANE transcripts", hjust =0) +
  annotate("text", x=-2.5,y=.9, label = "p-value = 2.322e-3", hjust = 0) +
  annotate("text", x = -2.5, y = 0.85, label = "PTC+ = 278", hjust = 0) +
  annotate("text", x = -2.5, y = 0.8, label = "PTC- = 253", hjust = 0) +
  geom_vline(xintercept = 0, color = "grey") + 
  geom_hline(yintercept = 0.5, color = "grey") +
  theme(legend.position = c(0.85,0.1))
MANE_sPTC_cdf
ggsave("SNRNP200_K562_sPTC_MANE_CDF.pdf", 
       plot = MANE_sPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
       ...
       device = "pdf",
       dpi = 300)
ggsave("C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/SNRNP200_K562_sPTC_MANE_CDF.pdf", 
       plot = MANE_sPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
...
write_csv(PTC_tpm, "C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/SNRNP200_PTC_MANE_TPM.csv")

#### Look at the effect when only WT filtering####
WTcounts <-as.data.frame(txi.kallisto.tsv$counts)

#Filtering out transcripts with mean tpm<1
WTcounts$mean=rowMeans(WTcounts[,c("SRR4421417","SRR4421418")], na.rm=TRUE) #Filter WT samples
...
write_csv(WTalltrans,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/SNRNP200_WTfilt_alltrans.csv")
write_csv(alltrans,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/SNRNP200_DBfilt_alltrans.csv")
write_csv(alltransSPTC_MANE,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/SNRNP200_PTC_alltrans.csv")
...
WT5counts$mean=rowMeans(WT5counts[,c("SRR4421417","SRR4421418")], na.rm=TRUE) #Filter WT samples
...
write_csv(WT5alltrans,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/SNRNP200_WT5filt_alltrans.csv")
