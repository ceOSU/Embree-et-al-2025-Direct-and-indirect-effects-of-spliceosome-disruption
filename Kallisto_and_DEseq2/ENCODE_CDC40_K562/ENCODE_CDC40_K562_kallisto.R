...
#Modified on: 6/2/2023
#Analyzing: CDC40_K562
...
#C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_CDC40_K562 --> working directory of the project
...
setwd(dir = "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_CDC40_K562/")
...
samples <- read_excel("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_CDC40_K562/samples.xlsx")
...
files1 <- file.path("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_CDC40_K562","koutput", samples$Run, "abundance.h5")
...
CDC40_K562transcripts=filter(listgenes, external_gene_name == "CDC40" )



CDC40_K562transcriptsWT<-CDC40_K562transcripts %>% dplyr::select(WT1,WT2, ensembl_transcript_id_version, external_gene_name, external_transcript_name)



CDC40_K562transcriptsWT$averagecounts = rowMeans(CDC40_K562transcriptsWT[,c("WT1","WT2")])
...
CDC40_K562transcriptsWT$percent= CDC40_K562transcriptsWT$averagecounts/sum(CDC40_K562transcriptsWT$averagecounts)



CDC40_K562transcriptsWT<-CDC40_K562transcriptsWT %>% arrange(averagecounts)
...
CDC40_K562transcriptsKD<-CDC40_K562transcripts %>% dplyr::select(KD1,KD2, ensembl_transcript_id_version, external_gene_name, external_transcript_name)



CDC40_K562transcriptsKD$averagecounts = rowMeans(CDC40_K562transcriptsKD[,c("KD1","KD2")])



CDC40_K562transcriptsKD$percent= CDC40_K562transcriptsKD$averagecounts/sum(CDC40_K562transcriptsKD$averagecounts)



CDC40_K562transcriptsKD<-CDC40_K562transcriptsKD %>% arrange(averagecounts)
...
plot(euler(s), quantities = TRUE,
     main="CDC40 knockout vs control")
...
NMDBio_cdf = ggplot(alltransbioNMD, aes(log2FoldChange, colour=transcript_biotype))+
  ...
  labs(y = "Cumulative Frequency", title = "CDC40 KD vs WT", subtitle = "NMD biotype")
NMDBio_cdf

#### analysis with robert's PTC+ and PTC- list ####
ENST_PTC.EPI.TFG <- read.delim("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_CDC40_K562/ENST_PTC-EPI-TFG.txt")
...
PTC_cdf = ggplot(alltransPTC, aes(log2FoldChange, colour=PTC.Status))+
  ...
  coord_cartesian(xlim = c(-3,3)) +
  scale_color_manual(values = met.brewer("Java",2)) +
  labs(y = "Cumulative Frequency", title = "CDC40 KD vs WT", subtitle = "PTC list")
PTC_cdf
...
NMD_transcripts = read.delim("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_CDC40_K562/NMD_transcript_list.txt")
...
NMD_cdf = ggplot(alltransNMD, aes(log2FoldChange, colour=NMD))+
  ...
  coord_cartesian(xlim = c(-3,3)) +
  scale_color_manual(values = met.brewer("Java",2)) +
  labs(y = "Cumulative Frequency", title = "CDC40 KD vs WT", subtitle = "NMD +/- list")
NMD_cdf
...
StringentNMD = read.csv("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_CDC40_K562/Stringent_NMD_list_CE")
...
sNMD_cdf = ggplot(alltransSTNMD, aes(log2FoldChange, colour=NMD))+
  ...
  coord_cartesian(xlim = c(-3,3)) +
  scale_color_manual(values = met.brewer("Java",2)) +
  labs(y = "Cumulative Frequency", title = "CDC40 KD vs WT", subtitle = "Stringent NMD list")
sNMD_cdf
...
sPTC = read.csv("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_CDC40_K562/Stringent_PTC_CE") 
alltransSPTC = inner_join(alltrans, sPTC, by = c("ENST.ID" = "transID")) #1194 PTC false 394 PTC true
sPTC_res <- wilcox.test(log2FoldChange ~ PTC, data = alltransSPTC,
                        exact = FALSE, alternative = "less")
count(alltransSPTC, PTC)
head(alltransSPTC)
sPTC_res #p-vlaue = 2.788e-07
...
sPTC_cdf = sPTC_cdf + annotate("text", x=-2.5,y=1, label = "CDC40 KD vs WT", size = 5, hjust = 0) +
  annotate("text", x=-2.5,y=.95, label = "Stringent PTC containing transcripts", hjust =0) +
  annotate("text", x=-2.5,y=.9, label = "p-value = 2.788e-7", hjust = 0) +
  annotate("text", x = -2.5, y = 0.85, label = "PTC+ = 394", hjust = 0) +
  annotate("text", x = -2.5, y = 0.8, label = "PTC- = 1194", hjust = 0) +
  geom_vline(xintercept = 0, color = "grey") + 
  geom_hline(yintercept = 0.5, color = "grey") +
  theme(legend.position = c(0.85,0.1))
sPTC_cdf
ggsave("CDC40_K562_sPTC_CDF.pdf", 
       plot = sPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
       ...
       device = "pdf",
       dpi = 300)
ggsave("C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/CDC40_K562_sPTC_CDF.pdf", 
       plot = sPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
...
CDC40_x = alltransSTNMD %>% filter(NMD == "TRUE")  %>% #Makes a data table of the top NMD transcript for each gene
  select(Gene,log2FoldChange,baseMean) %>% 
  rename(plusFC = log2FoldChange) %>% 
  group_by(Gene) %>% 
  slice_max(baseMean, n = 1)

CDC40_y  = alltransSTNMD %>% filter(NMD == "FALSE")  %>% #Makes a data table fo the top NMD- transcript for each gene
  select(Gene,log2FoldChange,baseMean) %>% 
  rename(minusFC = log2FoldChange) %>% 
  group_by(Gene) %>% 
  slice_max(baseMean, n = 1)

CDC40_scatter = full_join(CDC40_x, CDC40_y, by = "Gene", suffix = c("plus","minus")) #Combine the two top tables
CDC40_scatter = CDC40_scatter %>% filter(!is.na(minusFC)) %>% 
  filter(!is.na(plusFC)) #Removes any NA rows


dot = ggplot(data = CDC40_scatter)+
  ...
  labs(x = "Log2FC NMD+", y = "Log2FC NMD-", title = "CDC40 gene-level FC comparison")+
  theme_minimal()+
...
CDC40_NMD = alltrans_MANE %>% inner_join(NMD_factors, by = c("external_gene_name" = "NMDfactor"))
write.csv(CDC40_NMD, "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/NMD_analysis/CDC40_NMDfactor.csv",row.names = FALSE)
...
PE_res <- wilcox.test(log2FoldChange ~ isoform, data = PE_FC,
                      exact = FALSE, alternative = "less")
head(PE_FC)
count(PE_FC, isoform)# 93 PE 72  MANE
PE_res #p-vlaue = 0.3449
...
PE_cdf = ggplot(PE_FC, aes(log2FoldChange, colour=isoform))+
  ...
  labs(y = "Cumulative Frequency")
PE_cdf = PE_cdf + annotate("text", x=-2.5,y=1, label = "CDC40 KD vs WT", size = 5, hjust = 0) +
  annotate("text", x=-2.5,y=.95, label = "Transcripts with a poison exon", hjust =0) +
  annotate("text", x=-2.5,y=.9, label = "p-value = 0.3449", hjust = 0) +
  annotate("text", x = -2.5, y = 0.85, label = "PE = 93", hjust = 0) +
  annotate("text", x = -2.5, y = 0.8, label = "MANE = 72", hjust = 0) +
  geom_vline(xintercept = 0, color = "grey") + 
  geom_hline(yintercept = 0.5, color = "grey") +
  theme(legend.position = c(0.85,0.1))
PE_cdf
ggsave("CDC40_K562_PE_CDF.pdf", 
       plot = PE_cdf,
       scale = 1,
       width = 8,
       height = 6,
       ...
       device = "pdf",
       dpi = 300)
ggsave("C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/CDC40_K562_PE_CDF.pdf", 
       plot = PE_cdf,
       scale = 1,
       width = 8,
       height = 6,
...
PEa_res <- wilcox.test(log2FoldChange ~ isoform, data = PEa_FC,
                       exact = FALSE, alternative = "less")
head(PEa_FC)
count(PEa_FC, isoform)# 445 PE 235  MANE
PEa_res #p-vlaue = 0.2781
...
PEa_cdf = ggplot(PEa_FC, aes(log2FoldChange, colour=isoform))+
  ...
  scale_color_manual(values = plot_colors) +
  labs(y = "Cumulative Frequency")
PEa_cdf = PEa_cdf + annotate("text", x=-2.5,y=1, label = "CDC40 KD vs WT", size = 5, hjust = 0) +
  annotate("text", x=-2.5,y=.95, label = "Transcripts with a poison exon", hjust =0) +
  annotate("text", x=-2.5,y=.9, label = "p-value = 0.2781", hjust = 0) +
  annotate("text", x = -2.5, y = 0.85, label = "PE = 445", hjust = 0) +
  annotate("text", x = -2.5, y = 0.8, label = "MANE = 235", hjust = 0) +
  geom_vline(xintercept = 0, color = "grey") + 
  geom_hline(yintercept = 0.5, color = "grey") +
  theme(legend.position = c(0.85,0.1))
PEa_cdf
ggsave("CDC40_K562_PE_archive_CDF.pdf", 
       plot = PEa_cdf,
       scale = 1,
       width = 8,
       height = 6,
       ...
       device = "pdf",
       dpi = 300)
ggsave("C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/CDC40_K562_PEarchive_CDF.pdf", 
       plot = PEa_cdf,
       scale = 1,
       width = 8,
       height = 6,
...
sPTC_MANE_res <- wilcox.test(log2FoldChange ~ PTC, data = alltransSPTC_MANE,
                             exact = FALSE, alternative = "less")
count(alltransSPTC_MANE, PTC) # 268 PTC false 394 PTC true
sPTC_MANE_res #p-vlaue = 5.875e-6
...
MANE_sPTC_cdf = ggplot(alltransSPTC_MANE, aes(log2FoldChange, colour=PTC))+
  ...
  scale_color_manual(values = plot_colors) +
  labs(y = "Cumulative Frequency")
MANE_sPTC_cdf = MANE_sPTC_cdf + annotate("text", x=-2.5,y=1, label = "CDC40 KD vs WT", size = 5, hjust = 0) +
  annotate("text", x=-2.5,y=.95, label = "PTC containing and MANE transcripts", hjust =0) +
  annotate("text", x=-2.5,y=.9, label = "p-value = 5.875e-6", hjust = 0) +
  annotate("text", x = -2.5, y = 0.85, label = "PTC+ = 394", hjust = 0) +
  annotate("text", x = -2.5, y = 0.8, label = "PTC- = 268", hjust = 0) +
  geom_vline(xintercept = 0, color = "grey") + 
  geom_hline(yintercept = 0.5, color = "grey") +
  theme(legend.position = c(0.85,0.1))
MANE_sPTC_cdf
ggsave("CDC40_K562_sPTC_MANE_CDF.pdf", 
       plot = MANE_sPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
       ...
       device = "pdf",
       dpi = 300)
ggsave("C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/CDC40_K562_sPTC_MANE_CDF.pdf", 
       plot = MANE_sPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
...
write_csv(PTC_tpm, "CDC40_PTC_MANE_TPM.csv")
write_csv(PTC_tpm, "C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/CDC40_PTC_MANE_TPM.csv")
...
write_csv(WTalltrans,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/CDC40_WTfilt_alltrans.csv")
write_csv(alltrans,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/CDC40_DBfilt_alltrans.csv")
write_csv(alltransSPTC_MANE,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/CDC40_PTC_alltrans.csv")
...
write_csv(WT5alltrans,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/CDC40_WT5filt_alltrans.csv")
