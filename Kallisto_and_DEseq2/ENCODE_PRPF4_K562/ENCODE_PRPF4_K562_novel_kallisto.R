...
#Analyzing: PRPF4 Kallisto results from transcriptome including novel transcripts
...
# C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_PRPF4_K562/novel_kallisto --> working directory of the project
# KD1 --> Sample ID of first WT sample
# KD2 --> Sample ID of second WT sample
# WT1 --> Sample ID of first KD sample
# WT2 --> Sample ID of second KD sample
...
getwd()
samples <- read_excel("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_PRPF4_K562/PRPF4_samples.xlsx")
View(samples)


#### Load in the files ####
files1 <- file.path("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_PRPF4_K562/novel_kallisto","novel_koutput", samples$Run, "abundance.h5")
names(files1) <- paste0(c("WT1","WT2","KD1","KD2","KD3","KD4"))
files1
...
counts = counts %>% mutate(kdMean = rowMeans(dplyr::select(counts,c(KD1,KD2)), na.rm = TRUE) ,
...
PRPF4_novel_iso_features <- read_csv("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/ENCODE_PRPF4_ISAR/PRPF4_novel_iso_features.csv")
novel_genes = counts %>% filter(str_detect(ENSTID,"MST")) %>% 
  full_join(PRPF4_novel_iso_features, by = c("ENSTID" = "isoform_id"))
...
alltrans_novel = alltrans %>% filter(str_detect(ENST.ID,"MST")) %>% 
  left_join(PRPF4_novel_iso_features,by = c("ENST.ID" = "isoform_id"))#add in the annotation from ISAR for the novel isoforms
...
write_csv(alltransbio,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/PRPF4_NK_alltrans.csv")
...
write.csv(PRPF4_NMD, "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/NMD_analysis/PRPF4_nk_NMDfactor.csv",row.names = FALSE)
...
PRPF4_SC_impact = alltrans_MANE %>% inner_join(Splicing_factors, by = c("external_gene_name" = "Component"))
write.csv(PRPF4_SC_impact, "PRPF4_splicing_impact.csv",row.names = FALSE)
...
PTC_tpm = PTC_tpm %>% select(5,6,8:10,14)
write_csv(PTC_tpm, "PRPF4_PTC_MANE_TPM.csv")
write_csv(PTC_tpm, "C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/PRPF4_nk_PTC_MANE_TPM.csv")
