...
#Analyzing: PRPF4 shRNA KD RNA-seq from ENCODE
...
# C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_PRPF4_K562 --> working directory of the project
# SRR14837933 --> Sample ID of first WT sample
# SRR14837934 --> Sample ID of second WT sample
# SRR14844613 --> Sample ID of first KD sample
# SRR14844614 --> Sample ID of second KD sample
# SRR14844615 --> Sample ID of second KD sample
# SRR14844616 --> Sample ID of second KD sample
# PRPF4 --> Ensemble name of your gene of interest
...
samples <- read_excel("PRPF4_samples.xlsx")
View(samples)


#Load in the files
files1 <- file.path("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_PRPF4_K562","koutput", samples$Run, "abundance.h5")
files1
names(files1) <- paste0(c("SRR14837933","SRR14837934","SRR14844613","SRR14844614","SRR14844615","SRR14844616"))
...
counts$mean=rowMeans(counts[,c("SRR14837933","SRR14837934")], na.rm=TRUE) #Filter WT samples
counts=filter(counts, mean>=1)
counts$mean=rowMeans(counts[,c("SRR14844613","SRR14844614","SRR14844615","SRR14844616")], na.rm=TRUE) #Fiter KD samples
counts=filter(counts, mean>=1)
counts$mean <- NULL
...
colData <- data.frame(genotype = factor(c("WT","WT","KD","KD","KD","KD"))) #make sure these are in the right order that match your sample file
...
plot(euler(s), quantities = TRUE,
     main="PRPF4 knockout vs control")
...
NMDBio_cdf = ggplot(alltransbioNMD, aes(log2FoldChange, colour=transcript_biotype))+
  ...
  labs(y = "Cumulative Frequency", title = "PRPF4 KD vs WT", subtitle = "NMD biotype")
NMDBio_cdf

#analysis with robert's PTC+ and PTC- list
ENST_PTC.EPI.TFG <- read.delim("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_PRPF4_K562/ENST_PTC-EPI-TFG.txt")
...
PTC_cdf = ggplot(alltransPTC, aes(log2FoldChange, colour=PTC.Status))+
  ...
  coord_cartesian(xlim = c(-3,3)) +
  scale_color_manual(values = met.brewer("Java",2)) +
  labs(y = "Cumulative Frequency", title = "PRPF4 KD vs WT", subtitle = "PTC list")
PTC_cdf
...
NMD_transcripts = read.delim("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_PRPF4_K562/NMD_transcript_list.txt")
...
NMD_cdf = ggplot(alltransNMD, aes(log2FoldChange, colour=NMD))+
  ...
  coord_cartesian(xlim = c(-3,3)) +
  scale_color_manual(values = met.brewer("Java",2)) +
  labs(y = "Cumulative Frequency", title = "PRPF4 KD vs WT", subtitle = "NMD +/- list")
NMD_cdf
...
StringentNMD = read.csv("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_PRPF4_K562/Stringent_NMD_list_CE")
...
sNMD_cdf = ggplot(alltransSTNMD, aes(log2FoldChange, colour=NMD))+
  ...
  coord_cartesian(xlim = c(-3,3)) +
  scale_color_manual(values = met.brewer("Java",2)) +
  labs(y = "Cumulative Frequency", title = "PRPF4 KD vs WT", subtitle = "Stringent NMD list")
sNMD_cdf
...
sPTC = read.csv("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_PRPF4_K562/Stringent_PTC_CE") #
alltransSPTC = inner_join(alltrans, sPTC, by = c("ENST.ID" = "transID")) 
sPTC_res <- wilcox.test(log2FoldChange ~ PTC, data = alltransSPTC,
                        exact = FALSE, alternative = "less")
head(alltransSPTC)
count(alltransSPTC, PTC) #929 PTC false 320 PTC true
sPTC_res #p-vlaue = 0.03142
...
sPTC_cdf = sPTC_cdf + annotate("text", x=-2.5,y=1, label = "PRPF4 KD vs WT", size = 5, hjust = 0) +
  annotate("text", x=-2.5,y=.95, label = "Stringent PTC containing transcripts", hjust =0) +
  annotate("text", x=-2.5,y=.9, label = "p-value = 0.03142", hjust = 0) +
  annotate("text", x = -2.5, y = 0.85, label = "PTC+ = 320", hjust = 0) +
  annotate("text", x = -2.5, y = 0.8, label = "PTC- = 929", hjust = 0) +
  geom_vline(xintercept = 0, color = "grey") + 
  geom_hline(yintercept = 0.5, color = "grey") +
  theme(legend.position = c(0.85,0.1))
sPTC_cdf
ggsave("PRPF4_K562_sPTC_CDF.pdf", 
       plot = sPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
       ...
       device = "pdf",
       dpi = 300)
ggsave("C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/PRPF4_K562_sPTC_CDF.pdf", 
       plot = sPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
...
PRPF4_x = alltransSTNMD %>% filter(NMD == "TRUE")  %>% #Makes a data table of the top NMD transcript for each gene
  select(Gene,log2FoldChange,baseMean) %>% 
  rename(plusFC = log2FoldChange) %>% 
  group_by(Gene) %>% 
  slice_max(baseMean, n = 1)

PRPF4_y  = alltransSTNMD %>% filter(NMD == "FALSE")  %>% #Makes a data table fo the top NMD- transcript for each gene
  select(Gene,log2FoldChange,baseMean) %>% 
  rename(minusFC = log2FoldChange) %>% 
  group_by(Gene) %>% 
  slice_max(baseMean, n = 1)

PRPF4_scatter = full_join(PRPF4_x, PRPF4_y, by = "Gene", suffix = c("plus","minus")) #Combine the two top tables
PRPF4_scatter = PRPF4_scatter %>% filter(!is.na(minusFC)) %>% 
  filter(!is.na(plusFC)) #Removes any NA rows


dot = ggplot(data = PRPF4_scatter)+
  ...
  labs(x = "Log2FC NMD+", y = "Log2FC NMD-", title = "PRPF4 gene-level FC comparison")+
  theme_minimal()+
...
PRPF4_NMD = alltrans_MANE %>% inner_join(NMD_factors, by = c("external_gene_name" = "NMDfactor"))
write.csv(PRPF4_NMD, "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/NMD_analysis/PRPF4_NMDfactor.csv",row.names = FALSE)
...
count(shared_SPTC_trans, PTC)#222 PTC false 90 PTC true
shared_sPTC_res #p-vlaue = 0.2355
...
shared_sPTC_cdf = ggplot(shared_SPTC_trans, aes(log2FoldChange, colour=PTC))+
  ...
  coord_cartesian(xlim = c(-3,3)) +
  scale_color_manual(values = met.brewer("Java",2)) +
  labs(y = "Cumulative Frequency", title = "PRPF4 KD vs WT", subtitle = "Shared stringent PTC transcripts")
shared_sPTC_cdf = shared_sPTC_cdf + annotate("text", x=-2.5,y=1, label = "p-value = 0.2355") +
  annotate("text", x = -2.5, y = 0.95, label = "PTC+ = 90") +
  annotate("text", x = -2.5, y = 0.9, label = "PTC- = 222") +
  geom_vline(xintercept = 0, color = "grey") + 
  geom_hline(yintercept = 0.5, color = "grey")
shared_sPTC_cdf
ggsave("PRPF4_K562_shared_PTC_CDF.pdf", 
...
PE_res <- wilcox.test(log2FoldChange ~ isoform, data = PE_FC,
                      exact = FALSE, alternative = "less")
head(PE_FC)
count(PE_FC, isoform)# 68 PE  65 MANE
PE_res #p-vlaue = 0.3996
...
PE_cdf = ggplot(PE_FC, aes(log2FoldChange, colour=isoform))+
  ...
  labs(y = "Cumulative Frequency")
PE_cdf = PE_cdf + annotate("text", x=-2.5,y=1, label = "PRPF4 KD vs WT", size = 5, hjust = 0) +
  annotate("text", x=-2.5,y=.95, label = "Transcripts with a poison exon", hjust =0) +
  annotate("text", x=-2.5,y=.9, label = "p-value = 0.3996", hjust = 0) +
  annotate("text", x = -2.5, y = 0.85, label = "PE = 68", hjust = 0) +
  annotate("text", x = -2.5, y = 0.8, label = "MANE = 65", hjust = 0) +
  geom_vline(xintercept = 0, color = "grey") + 
  geom_hline(yintercept = 0.5, color = "grey") +
  theme(legend.position = c(0.85,0.1))
PE_cdf
ggsave("PRPF4_K562_PE_CDF.pdf", 
       plot = PE_cdf,
       scale = 1,
       width = 8,
       height = 6,
       ...
       device = "pdf",
       dpi = 300)
ggsave("C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/PRPF4_K562_PE_CDF.pdf", 
       plot = PE_cdf,
       scale = 1,
       width = 8,
       height = 6,
...
PEa_res <- wilcox.test(log2FoldChange ~ isoform, data = PEa_FC,
                       exact = FALSE, alternative = "less")
head(PEa_FC)
count(PEa_FC, isoform)# 379 PE 212  MANE
PEa_res #p-vlaue = 9.139e-3
...
PEa_cdf = ggplot(PEa_FC, aes(log2FoldChange, colour=isoform))+
  ...
  scale_color_manual(values = plot_colors) +
  labs(y = "Cumulative Frequency")
PEa_cdf = PEa_cdf + annotate("text", x=-2.5,y=1, label = "PRPF4 KD vs WT", size = 5, hjust = 0) +
  annotate("text", x=-2.5,y=.95, label = "Transcripts with a poison exon", hjust =0) +
  annotate("text", x=-2.5,y=.9, label = "p-value = 9.139e-3", hjust = 0) +
  annotate("text", x = -2.5, y = 0.85, label = "PE = 379", hjust = 0) +
  annotate("text", x = -2.5, y = 0.8, label = "MANE = 212", hjust = 0) +
  geom_vline(xintercept = 0, color = "grey") + 
  geom_hline(yintercept = 0.5, color = "grey") +
  theme(legend.position = c(0.85,0.1))
PEa_cdf
ggsave("PRPF4_K562_PE_archive_CDF.pdf", 
       plot = PEa_cdf,
       scale = 1,
       width = 8,
       height = 6,
       ...
       device = "pdf",
       dpi = 300)
ggsave("C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/PRPF4_K562_PEarchive_CDF.pdf", 
       plot = PEa_cdf,
       scale = 1,
       width = 8,
       height = 6,
...
sPTC_MANE_res <- wilcox.test(log2FoldChange ~ PTC, data = alltransSPTC_MANE,
                             exact = FALSE, alternative = "less")
count(alltransSPTC_MANE, PTC) # 245 PTC false 320 PTC true
sPTC_MANE_res #p-vlaue = 1.173e-5
...
MANE_sPTC_cdf = ggplot(alltransSPTC_MANE, aes(log2FoldChange, colour=PTC))+
  ...
  scale_color_manual(values = plot_colors) +
  labs(y = "Cumulative Frequency")
MANE_sPTC_cdf = MANE_sPTC_cdf + annotate("text", x=-2.5,y=1, label = "PRPF4 KD vs WT", size = 5, hjust = 0) +
  annotate("text", x=-2.5,y=.95, label = "PTC containing and MANE transcripts", hjust =0) +
  annotate("text", x=-2.5,y=.9, label = "p-value = 1.173e-5", hjust = 0) +
  annotate("text", x = -2.5, y = 0.85, label = "PTC+ = 320", hjust = 0) +
  ...
  theme(legend.position = c(0.85,0.1))
MANE_sPTC_cdf
ggsave("PRPF4_K562_sPTC_MANE_CDF.pdf", 
       plot = MANE_sPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
       ...
       device = "pdf",
       dpi = 300)
ggsave("C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/PRPF4_K562_sPTC_MANE_CDF.pdf", 
       plot = MANE_sPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
...
full_tpm = listgenes %>% rowwise() %>%  mutate(WTmean = mean(c_across(1:2)), KDmean = mean(c_across(3:6))) %>% ungroup() #Check that WT and KD are referring to the right columns
...
write_csv(PTC_tpm, "PRPF4_PTC_MANE_TPM.csv")
write_csv(PTC_tpm, "C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/PRPF4_PTC_MANE_TPM.csv")
write_csv(alltrans,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/PRPF4_DBfilt_alltrans.csv")
write_csv(alltransSPTC_MANE,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/PRPF4_PTC_alltrans.csv")

#### Look at the effect when only WT filtering####
WTcounts <-as.data.frame(txi.kallisto.tsv$counts)

#Filtering out transcripts with mean tpm<1
WTcounts$mean=rowMeans(WTcounts[,c("SRR14837933","SRR14837934")], na.rm=TRUE) #Filter WT samples
...
write_csv(WTalltrans,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/PRPF4_WTfilt_alltrans.csv")
write_csv(alltrans,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/PRPF4_DBfilt_alltrans.csv")
write_csv(alltransSPTC_MANE,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/PRPF4_PTC_alltrans.csv")
...
WT5counts$mean=rowMeans(WT5counts[,c("SRR14837933","SRR14837934")], na.rm=TRUE) #Filter WT samples
...
write_csv(WT5alltrans,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/PRPF4_WT5filt_alltrans.csv")
