...
#Analyzing: ENCODE CRISPR-KO of PRPF8
...
# C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_PRPF8_K562 --> working directory of the project
# SRR14848422 --> Sample ID of first WT sample
# SRR14848423 --> Sample ID of second WT sample
# SRR14842475 --> Sample ID of first KD sample
# SRR14842476 --> Sample ID of second KD sample
...
#### Load in the files ####
files1 <- file.path("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_PRPF8_K562","koutput", samples$Run, "abundance.h5")
names(files1) <- paste0(c("SRR14842475","SRR14842476","SRR14848422","SRR14848423"))
files1
...
counts$mean=rowMeans(counts[,c("SRR14848422","SRR14848423")], na.rm=TRUE) #Filter WT samples
counts=filter(counts, mean>=1)
counts$mean=rowMeans(counts[,c("SRR14842475","SRR14842476")], na.rm=TRUE) #Fiter KD samples
counts=filter(counts, mean>=1)
counts$mean <- NULL
...
plot(euler(s), quantities = TRUE,
     main="PRPF8 knockout vs control")
...
NMDBio_cdf = ggplot(alltransbioNMD, aes(log2FoldChange, colour=transcript_biotype))+
  ...
  labs(y = "Cumulative Frequency", title = "PRPF8 KD vs WT", subtitle = "NMD biotype")
NMDBio_cdf

#### analysis with robert's PTC+ and PTC- list ####
ENST_PTC.EPI.TFG <- read.delim("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_PRPF8_K562/ENST_PTC-EPI-TFG.txt")


alltransPTC<-inner_join(alltrans, ENST_PTC.EPI.TFG, by = c("ENST.ID" = "ENST.ID"))
...
PTC_cdf = ggplot(alltransPTC, aes(log2FoldChange, colour=PTC.Status))+
  ...
  coord_cartesian(xlim = c(-3,3)) +
  scale_color_manual(values = met.brewer("Java",2)) +
  labs(y = "Cumulative Frequency", title = "PRPF8 KD vs WT", subtitle = "PTC list")
PTC_cdf
...
sPTC_MANE_res <- wilcox.test(log2FoldChange ~ PTC, data = alltransSPTC_MANE,
                             exact = FALSE, alternative = "less")
count(alltransSPTC_MANE, PTC) # 241 PTC false 221 PTC true
sPTC_MANE_res #p-vlaue = 0.005456
...
MANE_sPTC_cdf = MANE_sPTC_cdf + annotate("text", x=-2.5,y=1, label = "PRPF8 KD vs WT", size = 5, hjust = 0) +
  annotate("text", x=-2.5,y=.95, label = "PTC containing and MANE transcripts", hjust =0) +
  annotate("text", x=-2.5,y=.9, label = "p-value = 5.456e-3", hjust = 0) +
  annotate("text", x = -2.5, y = 0.85, label = "PTC+ = 221", hjust = 0) +
  annotate("text", x = -2.5, y = 0.8, label = "MANE = 241", hjust = 0) +
  geom_vline(xintercept = 0, color = "grey") + 
  geom_hline(yintercept = 0.5, color = "grey") +
  theme(legend.position = c(0.85,0.1))
MANE_sPTC_cdf
ggsave("PRPF8_K562_sPTC_MANE_CDF.pdf", 
       plot = MANE_sPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
       ...
       device = "pdf",
       dpi = 300)
ggsave("C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/PRPF8_K562_sPTC_MANE_CDF.pdf", 
       plot = MANE_sPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
...
PRPF8_NMD = alltrans_MANE %>% inner_join(NMD_factors, by = c("external_gene_name" = "NMDfactor"))
write.csv(PRPF8_NMD, "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/NMD_analysis/PRPF8_NMDfactor.csv",row.names = FALSE)
...
PRPF8_SC_impact = alltrans_MANE %>% inner_join(Splicing_factors, by = c("external_gene_name" = "Component"))
write.csv(PRPF8_SC_impact, "PRPF8_splicing_impact.csv",row.names = FALSE)
...
write_csv(PTC_tpm, "C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/PRPF8_PTC_MANE_TPM.csv")

#### Look at the effect when only WT filtering####
WTcounts <-as.data.frame(txi.kallisto.tsv$counts)

#Filtering out transcripts with mean tpm<1
WTcounts$mean=rowMeans(WTcounts[,c("SRR14848422","SRR14848423")], na.rm=TRUE) #Filter WT samples
...
write_csv(WTalltrans,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/PRPF8_WTfilt_alltrans.csv")
write_csv(alltrans,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/PRPF8_DBfilt_alltrans.csv")
write_csv(alltransSPTC_MANE,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/PRPF8_PTC_alltrans.csv")
...
WT5counts$mean=rowMeans(WT5counts[,c("SRR14848422","SRR14848423")], na.rm=TRUE) #Filter WT samples
...
write_csv(WT5alltrans,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/PRPF8_WT5filt_alltrans.csv")
