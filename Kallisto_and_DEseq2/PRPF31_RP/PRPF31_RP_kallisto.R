...
#Modified on: 10/19/2023
#Analyzing: PRPF31  RP causing mutants in iPSC 
...
# C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/PRPF31_RP --> working directory of the project
# ERR2162252 --> Sample ID of first WT sample
# ERR2162253 --> Sample ID of second WT sample
# ERR2162254 --> Sample ID of third WT sample
# ERR2162264 --> Sample ID of first Disease (RP) sample
# ERR2162265 --> Sample ID of second Disease (RP) sample
# ERR2162266 --> Sample ID of third Disease (RP) sample
# PRPF31 --> Ensemble name of your gene of interest
...
samples <- read_excel("samples_iPSC.xlsx")
View(samples)


#### Load in the files ####
files1 <- file.path("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/PRPF31_RP","koutput", samples$Run, "abundance.h5")
names(files1) <- paste0(c("ERR2162264","ERR2162265","ERR2162266","ERR2162252","ERR2162253","ERR2162254"))
files1
...
counts$mean=rowMeans(counts[,c("ERR2162252","ERR2162253","ERR2162254")], na.rm=TRUE) #Filter WT samples
counts=filter(counts, mean>=1)
counts$mean=rowMeans(counts[,c("ERR2162264","ERR2162265","ERR2162266")], na.rm=TRUE) #Fiter RP samples
counts=filter(counts, mean>=1)
counts$mean <- NULL
...
colData <- data.frame(genotype = factor(c("RP","RP","RP","WT","WT","WT"))) #make sure these are in the right order that match your sample file
...
res_counts <- results(ddscounts, contrast = c("genotype","RP","WT"))
res_counts
...
head(res.sig)
plotMA(res_counts, padj = TRUE, ylim=c(-6,6), main ="MA plot: WT vs RP")
...
#Exporting files
write.csv(as.data.frame(res_counts), file = "RPvsWT_iPSC_full_results.csv")
write.csv(as.data.frame(res.sig), file = "2RPvsWT_iPSC_sig_results.csv")
write.csv(counts(ddscounts, normalized = T), file = "RPvsWT_iPSC_normalized_counts.csv")

res_counts1 = read.csv("RPvsWT_iPSC_full_results.csv", header = T)
...
#intersect between NMDgenes and significantly changing genes in RNAseq(wt vs RP) = 950
...
NMDBio_cdf = ggplot(alltransbioNMD, aes(log2FoldChange, colour=transcript_biotype))+
  ...
  labs(y = "Cumulative Frequency", title = "PRPF31 RP vs WT iPSC", subtitle = "NMD biotype")
NMDBio_cdf

#### analysis with robert's PTC+ and PTC- list ####
ENST_PTC.EPI.TFG <- read.delim("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/PRPF31_RP/ENST_PTC-EPI-TFG.txt")
...
PTC_cdf = ggplot(alltransPTC, aes(log2FoldChange, colour=PTC.Status))+
  ...
  coord_cartesian(xlim = c(-3,3)) +
  scale_color_manual(values = met.brewer("Java",2)) +
  labs(y = "Cumulative Frequency", title = "PRPF31 RP vs WT iPSC", subtitle = "PTC list")
PTC_cdf
...
NMD_transcripts = read.delim("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/PRPF31_RP/NMD_transcript_list.txt")
...
NMD_cdf = ggplot(alltransNMD, aes(log2FoldChange, colour=NMD))+
  ...
  coord_cartesian(xlim = c(-3,3)) +
  scale_color_manual(values = met.brewer("Java",2)) +
  labs(y = "Cumulative Frequency", title = "PRPF31 RP vs WT iPSC", subtitle = "NMD +/- list")
NMD_cdf
...
StringentNMD = read.csv("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/PRPF31_RP/Stringent_NMD_list_CE")
...
sNMD_cdf = ggplot(alltransSTNMD, aes(log2FoldChange, colour=NMD))+
  ...
  coord_cartesian(xlim = c(-3,3)) +
  scale_color_manual(values = met.brewer("Java",2)) +
  labs(y = "Cumulative Frequency", title = "PRPF31 RP vs WT iPSC", subtitle = "Stringent NMD list")
sNMD_cdf
...
sPTC = read.csv("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/PRPF31_RP/Stringent_PTC_CE") 
alltransSPTC = inner_join(alltrans, sPTC, by = c("ENST.ID" = "transID")) 
sPTC_res <- wilcox.test(log2FoldChange ~ PTC, data = alltransSPTC,
                        exact = FALSE, alternative = "less")
head(alltransSPTC)
count(alltransSPTC, PTC) # 921 PTC false 290 PTC true
sPTC_res #p-vlaue = 2.945e-8
...
sPTC_cdf = sPTC_cdf + annotate("text", x=-2.5,y=1, label = "PRPF31 RP vs WT iPSC", size = 5, hjust = 0) +
  annotate("text", x=-2.5,y=.95, label = "Stringent PTC containing transcripts", hjust =0) +
  annotate("text", x=-2.5,y=.9, label = "p-value = 2.945e-8", hjust = 0) +
  annotate("text", x = -2.5, y = 0.85, label = "PTC+ = 290", hjust = 0) +
  annotate("text", x = -2.5, y = 0.8, label = "PTC- = 921", hjust = 0) +
  geom_vline(xintercept = 0, color = "grey") + 
  geom_hline(yintercept = 0.5, color = "grey") +
  theme(legend.position = c(0.85,0.1))
sPTC_cdf
ggsave("PRPF31_iPSC_sPTC_CDF.pdf", 
       plot = sPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
       ...
       device = "pdf",
       dpi = 300)
ggsave("C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/PRPF31_iPSC_sPTC_CDF.pdf", 
       plot = sPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
...
PRPF31_x = alltransSTNMD %>% filter(NMD == "TRUE")  %>% #Makes a data table of the top NMD transcript for each gene
  select(Gene,log2FoldChange,baseMean) %>% 
  rename(plusFC = log2FoldChange) %>% 
  group_by(Gene) %>% 
  slice_max(baseMean, n = 1)

PRPF31_y  = alltransSTNMD %>% filter(NMD == "FALSE")  %>% #Makes a data table fo the top NMD- transcript for each gene
  select(Gene,log2FoldChange,baseMean) %>% 
  rename(minusFC = log2FoldChange) %>% 
  group_by(Gene) %>% 
  slice_max(baseMean, n = 1)

PRPF31_scatter = full_join(PRPF31_x, PRPF31_y, by = "Gene", suffix = c("plus","minus")) #Combine the two top tables
PRPF31_scatter = PRPF31_scatter %>% filter(!is.na(minusFC)) %>% 
  filter(!is.na(plusFC)) #Removes any NA rows


dot = ggplot(data = PRPF31_scatter)+
  ...
  labs(x = "Log2FC NMD+", y = "Log2FC NMD-", title = "PRPF31 iPSC gene-level FC comparison")+
  theme_minimal()+
...
PRPF31_NMD = alltrans_MANE %>% inner_join(NMD_factors, by = c("external_gene_name" = "NMDfactor"))
write.csv(PRPF31_NMD, "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/NMD_analysis/PRPF31_IPSC_NMDfactor.csv",row.names = FALSE)
...
count(shared_SPTC_trans, PTC)#212 PTC false 81 PTC true
shared_sPTC_res #p-vlaue = 8.821e-9
...
shared_sPTC_cdf = ggplot(shared_SPTC_trans, aes(log2FoldChange, colour=PTC))+
  ...
  coord_cartesian(xlim = c(-3,3)) +
  scale_color_manual(values = met.brewer("Java",2)) +
  labs(y = "Cumulative Frequency", title = "PRPF31 RP vs WT iPSC", subtitle = "Shared stringent PTC transcripts")
shared_sPTC_cdf = shared_sPTC_cdf + annotate("text", x=-2.5,y=1, label = "p-value = 8.821e-9") +
  annotate("text", x = -2.5, y = 0.95, label = "PTC+ = 81") +
  annotate("text", x = -2.5, y = 0.9, label = "PTC- = 212") +
  geom_vline(xintercept = 0, color = "grey") + 
  geom_hline(yintercept = 0.5, color = "grey")
shared_sPTC_cdf
ggsave("PRPF31_K562_shared_PTC_CDF.pdf", 
...
PE_res <- wilcox.test(log2FoldChange ~ isoform, data = PE_FC,
                      exact = FALSE, alternative = "less")
head(PE_FC)
count(PE_FC, isoform)# 66 PE  76 MANE
PE_res #p-vlaue = 2.128e-4
...
PE_cdf = ggplot(PE_FC, aes(log2FoldChange, colour=isoform))+
  ...
  labs(y = "Cumulative Frequency")
PE_cdf = PE_cdf + annotate("text", x=-2.5,y=1, label = "PRPF31 RP vs WT iPSC", size = 5, hjust = 0) +
  annotate("text", x=-2.5,y=.95, label = "Transcripts with a poison exon", hjust =0) +
  annotate("text", x=-2.5,y=.9, label = "p-value = 2.128e-4", hjust = 0) +
  annotate("text", x = -2.5, y = 0.85, label = "PE = 66", hjust = 0) +
  annotate("text", x = -2.5, y = 0.8, label = "MANE = 76", hjust = 0) +
  geom_vline(xintercept = 0, color = "grey") + 
  geom_hline(yintercept = 0.5, color = "grey") +
  theme(legend.position = c(0.85,0.1))
PE_cdf
ggsave("PRPF31_iPSC_PE_CDF.pdf", 
       plot = PE_cdf,
       scale = 1,
       width = 8,
       height = 6,
       ...
       device = "pdf",
       dpi = 300)
ggsave("C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/PRPF31_iPSC_PE_CDF.pdf", 
       plot = PE_cdf,
       scale = 1,
       width = 8,
       height = 6,
...
PEa_res <- wilcox.test(log2FoldChange ~ isoform, data = PEa_FC,
                       exact = FALSE, alternative = "less")
head(PEa_FC)
count(PEa_FC, isoform)# 326 PE  257 MANE
PEa_res #p-vlaue = 3.904e-5
...
PEa_cdf = ggplot(PEa_FC, aes(log2FoldChange, colour=isoform))+
  ...
  scale_color_manual(values = plot_colors) +
  labs(y = "Cumulative Frequency")
PEa_cdf = PEa_cdf + annotate("text", x=-2.5,y=1, label = "PRPF31 RP iPSC vs WT", size = 5, hjust = 0) +
  annotate("text", x=-2.5,y=.95, label = "Transcripts with a poison exon", hjust =0) +
  annotate("text", x=-2.5,y=.9, label = "p-value = 3.904e-5", hjust = 0) +
  annotate("text", x = -2.5, y = 0.85, label = "PE = 326", hjust = 0) +
  annotate("text", x = -2.5, y = 0.8, label = "MANE = 257", hjust = 0) +
  geom_vline(xintercept = 0, color = "grey") + 
  geom_hline(yintercept = 0.5, color = "grey") +
  theme(legend.position = c(0.85,0.1))
PEa_cdf
ggsave("PRPF31_K562_PE_archive_CDF.pdf", 
       plot = PEa_cdf,
       scale = 1,
       width = 8,
       height = 6,
       ...
       device = "pdf",
       dpi = 300)
ggsave("C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/PRPF31_IPSC_PEarchive_CDF.pdf", 
       plot = PEa_cdf,
       scale = 1,
       width = 8,
       height = 6,
...
PRPF31_RP_iPSC_SC_impact = alltrans_MANE %>% inner_join(Splicing_factors, by = c("external_gene_name" = "Component"))
write.csv(PRPF31_RP_iPSC_SC_impact, "GOI_splicing_impact.csv",row.names = FALSE)
...
sPTC_MANE_res <- wilcox.test(log2FoldChange ~ PTC, data = alltransSPTC_MANE,
                             exact = FALSE, alternative = "less")
count(alltransSPTC_MANE, PTC) # 285 PTC false 290 PTC true
sPTC_MANE_res #p-vlaue = 3.401e-3
...
MANE_sPTC_cdf = ggplot(alltransSPTC_MANE, aes(log2FoldChange, colour=PTC))+
  ...
  scale_color_manual(values = plot_colors) +
  labs(y = "Cumulative Frequency")
MANE_sPTC_cdf = MANE_sPTC_cdf + annotate("text", x=-2.5,y=1, label = "PRPF31 RP vs WT iPSC", size = 5, hjust = 0) +
  annotate("text", x=-2.5,y=.95, label = "PTC containing and MANE transcripts", hjust =0) +
  annotate("text", x=-2.5,y=.9, label = "p-value = 3.401e-3", hjust = 0) +
  annotate("text", x = -2.5, y = 0.85, label = "PTC+ = 290", hjust = 0) +
  annotate("text", x = -2.5, y = 0.8, label = "MANE = 285", hjust = 0) +
  geom_vline(xintercept = 0, color = "grey") + 
  geom_hline(yintercept = 0.5, color = "grey") +
  theme(legend.position = c(0.85,0.1))
MANE_sPTC_cdf
ggsave("PRPF31_iPSC_sPTC_MANE_CDF.pdf", 
       plot = MANE_sPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
       ...
       device = "pdf",
       dpi = 300)
ggsave("C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/PRPF31_iPSC_sPTC_MANE_CDF.pdf", 
       plot = MANE_sPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
...
full_tpm = listgenes %>% rowwise() %>%  mutate(WTmean = mean(c_across(4:6)), KDmean = mean(c_across(1:3))) %>% ungroup() #Check that WT and KD are referring to the right columns
full_tpm = full_tpm %>% mutate(ENSTID = str_remove(ensembl_transcript_id_version, "\\..*"))
full_tpm = full_tpm %>% select(8,10:12)
PTC_tpm = full_tpm %>% right_join(sPTC_MANE, by = c("ENSTID" = "transID"))
PTC_tpm = PTC_tpm %>% select(2:4,6,7)
write_csv(PTC_tpm, "PRPF31_IPSC_PTC_MANE_TPM.csv")
write_csv(PTC_tpm, "C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/PRPF31_IPSC_PTC_MANE_TPM.csv")

#### Look at the effect when only WT filtering####
WTcounts <-as.data.frame(txi.kallisto.tsv$counts)

#Filtering out transcripts with mean tpm<1
WTcounts$mean=rowMeans(WTcounts[,c("ERR2162252","ERR2162253","ERR2162254")], na.rm=TRUE) #Filter WT samples
...
WTres_counts <- results(WTddscounts, contrast = c("genotype","RP","WT"))
WTres_counts
...
write_csv(WTalltrans,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/PRPF31_IPSC_WTfilt_alltrans.csv")
write_csv(alltrans,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/PRPF31_IPSC_DBfilt_alltrans.csv")
write_csv(alltransSPTC_MANE,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/PRPF31_IPSC_PTC_alltrans.csv")
...
WT5counts$mean=rowMeans(WT5counts[,c("ERR2162252","ERR2162253","ERR2162254")], na.rm=TRUE) #Filter WT samples
...
WT5res_counts <- results(WT5ddscounts, contrast = c("genotype","RP","WT"))
WT5res_counts
...
write_csv(WT5alltrans,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/PRPF31_IPSC_WT5filt_alltrans.csv")
