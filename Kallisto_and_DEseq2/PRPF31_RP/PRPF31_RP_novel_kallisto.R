...
#Analyzing: PRPF31_RP Kallisto results from transcriptome including novel transcripts
...
# C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/PRPF31_RP/novel_kallisto --> working directory of the project
# RP1 --> Sample ID of first WT sample
# RP2 --> Sample ID of second WT sample
# WT1 --> Sample ID of first RP sample
# WT2 --> Sample ID of second RP sample
# PRPF31_RP --> Ensemble name of your gene of interest
...
getwd()
samples <- read_excel("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/PRPF31_RP/samples_iPSC.xlsx")
View(samples)


#### Load in the files ####
files1 <- file.path("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/PRPF31_RP/novel_kallisto","novel_koutput", samples$Run, "abundance.h5")
names(files1) <- paste0(c("RP1","RP2","RP3","WT1","WT2","WT3"))
files1
...
counts = counts %>% mutate(kdMean = rowMeans(dplyr::select(counts,c(RP1,RP2)), na.rm = TRUE) ,
                           wtMean = rowMeans(dplyr::select(counts,c(WT1,WT2)), na.rm = TRUE)) %>% 
  filter(wtMean >= 1 | kdMean >= 1) #Filter to the transcripts with TPM >= 1 in either WT or RP
...
PRPF31_RP_novel_iso_features <- read_csv("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/PRPF31_RP_ISAR/PRPF31_RP_novel_iso_features.csv")
novel_genes = counts %>% filter(str_detect(ENSTID,"MST")) %>% 
  left_join(PRPF31_RP_novel_iso_features, by = c("ENSTID" = "isoform_id")) %>% 
  filter(!is.na(PTC))
...
PCA
ggsave("PRPF31_RP_novel_PCA.pdf",
       plot = PCA,
       device = pdf,
       width = 5,
       height = 5,
...
#Exporting files
write.csv(as.data.frame(res_counts), file = "RPvsWT_full_results.csv")
write.csv(as.data.frame(res.sig), file = "2RPvsWT_sig_results.csv")
write.csv(counts(ddscounts, normalized = T), file = "RPvsWT_normalized_counts.csv")
...
plot(euler(s), quantities = TRUE,
     main="PRPF31_RP knockout vs control")
...
alltrans_novel = alltrans %>% filter(str_detect(ENST.ID,"MST")) %>% 
  left_join(PRPF31_RP_novel_iso_features,by = c("ENST.ID" = "isoform_id"))#add in the annotation from ISAR for the novel isoforms
...
write_csv(alltransbio,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/PRPF31_RP_NK_alltrans.csv")
...
NMDBio_cdf = ggplot(alltransbioNMD, aes(log2FoldChange, colour=transcript_biotype))+
  ...
  labs(y = "Cumulative Frequency", title = "AQR RP vs WT", subtitle = "NMD biotype")
NMDBio_cdf

#### analysis with robert's PTC+ and PTC- list ####
ENST_PTC.EPI.TFG <- read.delim("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/Bioinformatics template/spliceosomeKD_analysis/ENST_PTC-EPI-TFG.txt")
...
PTC_cdf = ggplot(alltransPTC, aes(log2FoldChange, colour=PTC.Status))+
  ...
  coord_cartesian(xlim = c(-3,3)) +
  scale_color_manual(values = met.brewer("Java",2)) +
  labs(y = "Cumulative Frequency", title = "PRPF31_RP RP vs WT", subtitle = "PTC list")
PTC_cdf
...
plot_colors = c("MANE" = "#673272", "NMD" = "#EA7428")



##sPTC analysis using the MANE list
sPTC_MANE = read.csv("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/Bioinformatics template/PTC_list_creation/SMG_PTC_MANE.csv")
alltransSPTC_MANE = inner_join(alltrans, sPTC_MANE, by = c("ENST.ID"= "ensembl_transcript_id")) 
sPTC_MANE_res <- wilcox.test(log2FoldChange ~ isoform, data = alltransSPTC_MANE,
                             exact = FALSE, alternative = "less")
count(alltransSPTC_MANE, isoform) # isoform false  isoform true
sPTC_MANE_res #p-vlaue = 


MANE_sPTC_cdf = ggplot(alltransSPTC_MANE, aes(log2FoldChange, colour=isoform))+
  stat_ecdf(linewidth=2)+
  theme_minimal()+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(color = "grey"))+
  coord_cartesian(xlim = c(-3,3)) +
...
MANE_sPTC_cdf = MANE_sPTC_cdf + annotate("text", x=-2.5,y=1, label = "PRPF31_RP RP vs WT", size = 5, hjust = 0) +
  annotate("text", x=-2.5,y=.95, label = "NMD and MANE transcripts", hjust =0) +
...
MANE_sPTC_cdf
ggsave("PRPF31_RP_sPTC_MANE_CDF.pdf", 
       plot = MANE_sPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
       ...
       device = "pdf",
       dpi = 300)
ggsave("C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/PRPF31_RP_sPTC_MANE_CDF.pdf", 
       plot = MANE_sPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
...
PRPF31_RP_NMD = alltrans_MANE %>% inner_join(NMD_factors, by = c("external_gene_name" = "NMDfactor"))
write.csv(PRPF31_RP_NMD, "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/NMD_analysis/PRPF31_RP_nk_NMDfactor.csv",row.names = FALSE)
...
PRPF31_RP_SC_impact = alltrans_MANE %>% inner_join(Splicing_factors, by = c("external_gene_name" = "Component"))
write.csv(PRPF31_RP_SC_impact, "PRPF31_RP_splicing_impact.csv",row.names = FALSE)



#### Pull the TPM of genes for the NMD TPM graph ####
#Check WT and RP are refering to the right columns in full_genelist
write_csv(full_genelist, "C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/PRPF31_RP_nk_TPM.csv")
