...
#Analyzing: PRPF8_RP Kallisto results from transcriptome including novel transcripts
...
# C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/PRPF8_RP/novel_kallisto --> working directory of the project
...
# PRPF8_RP --> Ensemble name of your gene of interest
...
getwd()
samples <- read_excel("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/PRPF8_RP/samples_Fibo.xlsx")
View(samples)


#### Load in the files ####
files1 <- file.path("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/PRPF8_RP/novel_kallisto","novel_koutput", samples$Run, "abundance.h5")
...
counts = counts %>% mutate(kdMean = rowMeans(dplyr::select(counts,c(RP1,RP2,RP3)), na.rm = TRUE) ,
                           wtMean = rowMeans(dplyr::select(counts,c(WT1,WT2,WT3)), na.rm = TRUE)) %>% 
...
PRPF8_RP_novel_iso_features <- read_csv("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/PRPF8_RP_ISAR/PRPF8_RP_novel_iso_features.csv")
novel_genes = counts %>% filter(str_detect(ENSTID,"MST")) %>% 
  left_join(PRPF8_RP_novel_iso_features, by = c("ENSTID" = "isoform_id")) %>% 
  filter(!is.na(PTC))
full_genelist = anno_genes %>% full_join(novel_genes, by = c("ENSTID",
                                                             "ensembl_gene_id" = "gene_id",
                                                             "external_gene_name" = "gene_name",
                                                             "WT1","WT2","WT3","RP1","RP2","RP3",
...
plot(euler(s), quantities = TRUE,
     main="PRPF8_RP knockout vs control")
...
alltrans_novel = alltrans %>% filter(str_detect(ENST.ID,"MST")) %>% 
  left_join(PRPF8_RP_novel_iso_features,by = c("ENST.ID" = "isoform_id"))#add in the annotation from ISAR for the novel isoforms
...
write_csv(alltransbio,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/PRPF8_RP_NK_alltrans.csv")
...
PTC_cdf = ggplot(alltransPTC, aes(log2FoldChange, colour=PTC.Status))+
  ...
  coord_cartesian(xlim = c(-3,3)) +
  scale_color_manual(values = met.brewer("Java",2)) +
  labs(y = "Cumulative Frequency", title = "PRPF8_RP RP vs WT", subtitle = "PTC list")
PTC_cdf
...
MANE_sPTC_cdf = MANE_sPTC_cdf + annotate("text", x=-2.5,y=1, label = "PRPF8_RP RP vs WT", size = 5, hjust = 0) +
...
MANE_sPTC_cdf
ggsave("PRPF8_RP_sPTC_MANE_CDF.pdf", 
       plot = MANE_sPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
       ...
       device = "pdf",
       dpi = 300)
ggsave("C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/PRPF8_RP_sPTC_MANE_CDF.pdf", 
       plot = MANE_sPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
...
PRPF8_RP_NMD = alltrans_MANE %>% inner_join(NMD_factors, by = c("external_gene_name" = "NMDfactor"))
write.csv(PRPF8_RP_NMD, "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/NMD_analysis/PRPF8_RP_nk_NMDfactor.csv",row.names = FALSE)
...
PRPF8_RP_SC_impact = alltrans_MANE %>% inner_join(Splicing_factors, by = c("external_gene_name" = "Component"))
write.csv(PRPF8_RP_SC_impact, "PRPF8_RP_splicing_impact.csv",row.names = FALSE)



#### Pull the TPM of genes for the NMD TPM graph ####
#Check WT and RP are refering to the right columns in full_genelist
write_csv(full_genelist, "C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/PRPF8_RP_nk_TPM.csv")
