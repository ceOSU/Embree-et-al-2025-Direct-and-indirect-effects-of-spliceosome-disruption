#Written by: Manu Sanjeev
#Modified by: Caleb Embree
#Modified on: 12/12/2021
#Analyzing: 
...
# C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_SNRPC_K562_KO --> working directory of the project
# SRR14838740 --> Sample ID of first WT sample
# SRR14838741 --> Sample ID of second WT sample
# SRR14845144 --> Sample ID of first KO sample
# SRR14845145 --> Sample ID of second KO sample
# SNRPC --> Ensemble name of your gene of interest
...
setwd("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_SNRPC_K562_KO")

#Load File containing sample types

samples <- read_excel("snRPC_K562_samples.xlsx")
View(samples)


#Load in the files
files1 <- file.path("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_SNRPC_K562_KO","koutput", samples$Run, "abundance.h5")
files1
names(files1) <- paste0(c("SRR14838740","SRR14838741","SRR14845144","SRR14845145"))
...
counts$mean=rowMeans(counts[,c("SRR14838740","SRR14838741")], na.rm=TRUE) #Filter WT samples
counts=filter(counts, mean>=1)
counts$mean=rowMeans(counts[,c("SRR14845144","SRR14845145")], na.rm=TRUE) #Fiter KD samples
counts=filter(counts, mean>=1)
counts$mean <- NULL
...
SNRPCtranscripts=filter(listgenes, external_gene_name == "SNRPC" )
SNRPCtranscriptsWT<-SNRPCtranscripts %>% dplyr::select(SRR14838740,SRR14838741, ensembl_transcript_id_version, external_gene_name, external_transcript_name)
SNRPCtranscriptsWT$averagecounts = rowMeans(SNRPCtranscriptsWT[,c("SRR14838740","SRR14838741")])
#put in specific library names above
SNRPCtranscriptsWT$percent= SNRPCtranscriptsWT$averagecounts/sum(SNRPCtranscriptsWT$averagecounts)
SNRPCtranscriptsWT<-SNRPCtranscriptsWT %>% arrange(averagecounts)

#Repeat for the KD transcripts
SNRPCtranscriptsKD<-SNRPCtranscripts %>% dplyr::select(SRR14845144,SRR14845145, ensembl_transcript_id_version, external_gene_name, external_transcript_name)
SNRPCtranscriptsKD$averagecounts = rowMeans(SNRPCtranscriptsKD[,c("SRR14845144","SRR14845145")])
SNRPCtranscriptsKD$percent= SNRPCtranscriptsKD$averagecounts/sum(SNRPCtranscriptsKD$averagecounts)
SNRPCtranscriptsKD<-SNRPCtranscriptsKD %>% arrange(averagecounts)
...
plot(euler(s), quantities = TRUE,
     main="SNRPC knockout vs control")
...
NMDBio_cdf = ggplot(alltransbioNMD, aes(log2FoldChange, colour=transcript_biotype))+
  ...
  labs(y = "Cumulative Frequency", title = "SNRPC KD vs WT", subtitle = "NMD biotype")
NMDBio_cdf

#analysis with robert's PTC+ and PTC- list
ENST_PTC.EPI.TFG <- read.delim("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_SNRPC_K562_KO/ENST_PTC-EPI-TFG.txt")
...
PTC_cdf = ggplot(alltransPTC, aes(log2FoldChange, colour=PTC.Status))+
  ...
  coord_cartesian(xlim = c(-3,3)) +
  scale_color_manual(values = met.brewer("Java",2)) +
  labs(y = "Cumulative Frequency", title = "SNRPC KD vs WT", subtitle = "PTC list")
PTC_cdf
...
NMD_transcripts = read.delim("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_SNRPC_K562_KO/NMD_transcript_list.txt")
...
NMD_cdf = ggplot(alltransNMD, aes(log2FoldChange, colour=NMD))+
  ...
  coord_cartesian(xlim = c(-3,3)) +
  scale_color_manual(values = met.brewer("Java",2)) +
  labs(y = "Cumulative Frequency", title = "SNRPC KD vs WT", subtitle = "NMD +/- list")
NMD_cdf
...
StringentNMD = read.csv("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_SNRPC_K562_KO/Stringent_NMD_list_CE")
...
sNMD_cdf = ggplot(alltransSTNMD, aes(log2FoldChange, colour=NMD))+
  ...
  coord_cartesian(xlim = c(-3,3)) +
  scale_color_manual(values = met.brewer("Java",2)) +
  labs(y = "Cumulative Frequency", title = "SNRPC KD vs WT", subtitle = "Stringent NMD list")
sNMD_cdf
...
sPTC = read.csv("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_SNRPC_K562_KO/Stringent_PTC_CE") #
alltransSPTC = inner_join(alltrans, sPTC, by = c("ENST.ID" = "transID")) #844 PTC false 286 PTC true
count(alltransSPTC, PTC)
sPTC_res <- wilcox.test(log2FoldChange ~ PTC, data = alltransSPTC,
                        exact = FALSE, alternative = "less")
head(alltransSPTC)
sPTC_res #p-vlaue = 0.989
...
sPTC_cdf = sPTC_cdf + annotate("text", x=-2.5,y=1, label = "SNRPC KO vs WT", size = 5, hjust = 0) +
  annotate("text", x=-2.5,y=.95, label = "Stringent PTC containing transcripts", hjust =0) +
  annotate("text", x=-2.5,y=.9, label = "p-value = 0.989", hjust = 0) +
  annotate("text", x = -2.5, y = 0.85, label = "PTC+ = 286", hjust = 0) +
  annotate("text", x = -2.5, y = 0.8, label = "PTC- = 844", hjust = 0) +
  geom_vline(xintercept = 0, color = "grey") + 
  geom_hline(yintercept = 0.5, color = "grey") +
  theme(legend.position = c(0.85,0.1))
sPTC_cdf
ggsave("SNRPC_K562_sPTC_CDF.pdf", 
       plot = sPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
       ...
       device = "pdf",
       dpi = 300)
ggsave("C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/SNRPC_K562_sPTC_CDF.pdf", 
       plot = sPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
...
SNRPC_x = alltransSTNMD %>% filter(NMD == "TRUE")  %>% #Makes a data table of the top NMD transcript for each gene
  select(Gene,log2FoldChange,baseMean) %>% 
  rename(plusFC = log2FoldChange) %>% 
  group_by(Gene) %>% 
  slice_max(baseMean, n = 1)

SNRPC_y  = alltransSTNMD %>% filter(NMD == "FALSE")  %>% #Makes a data table fo the top NMD- transcript for each gene
  select(Gene,log2FoldChange,baseMean) %>% 
  rename(minusFC = log2FoldChange) %>% 
  group_by(Gene) %>% 
  slice_max(baseMean, n = 1)

SNRPC_scatter = full_join(SNRPC_x, SNRPC_y, by = "Gene", suffix = c("plus","minus")) #Combine the two top tables
SNRPC_scatter = SNRPC_scatter %>% filter(!is.na(minusFC)) %>% 
  filter(!is.na(plusFC)) #Removes any NA rows


dot = ggplot(data = SNRPC_scatter)+
  ...
  labs(x = "Log2FC NMD+", y = "Log2FC NMD-", title = "SNRPC gene-level FC comparison")+
  theme_minimal()+
...
ASgenes_SNRPC <- read_delim("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/rMATS/ENCODE_SNRPC/ASgenes_SNRPC.csv", 
                          delim = ";", escape_double = FALSE, trim_ws = TRUE)
ASgenes_SNRPC = ASgenes_SNRPC %>% select(GeneID,geneSymbol)
...
sPTC_AS = sPTC_gene %>% anti_join(ASgenes_SNRPC, by = c("ensembl_gene_id" = "GeneID"))
count(sPTC_AS, PTC) #PTC false= 455 PTC true= 115
alltransAS = inner_join(alltrans, sPTC_AS, by = c("ENST.ID" = "ensembl_transcript_id"))
count(alltransAS, PTC) # PTC false= 178 PTC true= 47


asPTC_res <- wilcox.test(log2FoldChange ~ PTC, data = alltransAS,
                         exact = FALSE, alternative = "less")
head(alltransAS)
asPTC_res #p-vlaue = 0.4514
...
asPTC_cdf = ggplot(alltransAS, aes(log2FoldChange, colour=PTC))+
  ...
  coord_cartesian(xlim = c(-3,3)) +
  scale_color_manual(values = met.brewer("Java",2)) +
  labs(y = "Cumulative Frequency", title = "SNRPC KD vs WT", subtitle = "Non-AS transcripts")
asPTC_cdf = asPTC_cdf + annotate("text", x=-2.5,y=1, label = "p-value = 0.4514") +
  annotate("text", x = -2.5, y = 0.95, label = "PTC+ = 47") +
  annotate("text", x = -2.5, y = 0.9, label = "PTC- = 178") +
  geom_vline(xintercept = 0, color = "grey") + 
  geom_hline(yintercept = 0.5, color = "grey")
asPTC_cdf
ggsave("SNRPC_K562_asPTC_CDF.pdf", 
       plot = asPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
...
SNRPC_NMD = alltrans_MANE %>% inner_join(NMD_factors, by = c("external_gene_name" = "NMDfactor"))
write.csv(SNRPC_NMD, "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/NMD_analysis/SNRPC_NMDfactor.csv",row.names = FALSE)
...
PE_res <- wilcox.test(log2FoldChange ~ isoform, data = PE_FC,
                      exact = FALSE, alternative = "less")
head(PE_FC)
count(PE_FC, isoform)# 62 PE 65  MANE
PE_res #p-vlaue = 0.9996
...
PE_cdf = ggplot(PE_FC, aes(log2FoldChange, colour=isoform))+
  ...
  labs(y = "Cumulative Frequency")
PE_cdf = PE_cdf + annotate("text", x=-2.5,y=1, label = "GOI KD vs WT", size = 5, hjust = 0) +
  annotate("text", x=-2.5,y=.95, label = "Transcripts with a poison exon", hjust =0) +
  annotate("text", x=-2.5,y=.9, label = "p-value = 0.9996", hjust = 0) +
  annotate("text", x = -2.5, y = 0.85, label = "PE = 62", hjust = 0) +
  annotate("text", x = -2.5, y = 0.8, label = "MANE = 65", hjust = 0) +
  geom_vline(xintercept = 0, color = "grey") + 
  geom_hline(yintercept = 0.5, color = "grey") +
  theme(legend.position = c(0.85,0.1))
PE_cdf
ggsave("GOI_K562_PE_CDF.pdf", 
       plot = PE_cdf,
       scale = 1,
       width = 8,
       height = 6,
       ...
       device = "pdf",
       dpi = 300)
ggsave("C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/GOI_K562_PE_CDF.pdf", 
       plot = PE_cdf,
       scale = 1,
       width = 8,
       height = 6,
...
PEa_res <- wilcox.test(log2FoldChange ~ isoform, data = PEa_FC,
                       exact = FALSE, alternative = "less")
head(PEa_FC)
count(PEa_FC, isoform)# 328 PE 215  MANE
PEa_res #p-vlaue = 1
...
PEa_cdf = ggplot(PEa_FC, aes(log2FoldChange, colour=isoform))+
  ...
  scale_color_manual(values = plot_colors) +
  labs(y = "Cumulative Frequency")
PEa_cdf = PEa_cdf + annotate("text", x=-2.5,y=1, label = "SNRPC KD vs WT", size = 5, hjust = 0) +
  annotate("text", x=-2.5,y=.95, label = "Transcripts with a poison exon", hjust =0) +
  annotate("text", x=-2.5,y=.9, label = "p-value = 1", hjust = 0) +
  annotate("text", x = -2.5, y = 0.85, label = "PE = 328", hjust = 0) +
  annotate("text", x = -2.5, y = 0.8, label = "MANE = 215", hjust = 0) +
  geom_vline(xintercept = 0, color = "grey") + 
  geom_hline(yintercept = 0.5, color = "grey") +
  theme(legend.position = c(0.85,0.1))
PEa_cdf
ggsave("SNRPC_K562_PE_archive_CDF.pdf", 
       plot = PEa_cdf,
       scale = 1,
       width = 8,
       height = 6,
       ...
       device = "pdf",
       dpi = 300)
ggsave("C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/SNRPC_K562_PEarchive_CDF.pdf", 
       plot = PEa_cdf,
       scale = 1,
       width = 8,
       height = 6,
...
sPTC_MANE = read.csv("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_SNRPC_K562_KO/Stringent_PTC_MANE_CE.csv")
...
sPTC_MANE_res <- wilcox.test(log2FoldChange ~ PTC, data = alltransSPTC_MANE,
                             exact = FALSE, alternative = "less")
count(alltransSPTC_MANE, PTC) # 245 PTC false 286 PTC true
sPTC_MANE_res #p-vlaue = 0.9998
...
MANE_sPTC_cdf = ggplot(alltransSPTC_MANE, aes(log2FoldChange, colour=PTC))+
  ...
  scale_color_manual(values = plot_colors) +
  labs(y = "Cumulative Frequency")
MANE_sPTC_cdf = MANE_sPTC_cdf + annotate("text", x=-2.5,y=1, label = "SNRPC KD vs WT", size = 5, hjust = 0) +
  annotate("text", x=-2.5,y=.95, label = "PTC containing and MANE transcripts", hjust =0) +
  annotate("text", x=-2.5,y=.9, label = "p-value = 0.9998", hjust = 0) +
  annotate("text", x = -2.5, y = 0.85, label = "PTC+ = 286", hjust = 0) +
  annotate("text", x = -2.5, y = 0.8, label = "PTC- = 245", hjust = 0) +
  geom_vline(xintercept = 0, color = "grey") + 
  geom_hline(yintercept = 0.5, color = "grey") +
  theme(legend.position = c(0.85,0.1))
MANE_sPTC_cdf
ggsave("SNRPC_K562_sPTC_MANE_CDF.pdf", 
       plot = MANE_sPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
       ...
       device = "pdf",
       dpi = 300)
ggsave("C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/SNRPC_K562_sPTC_MANE_CDF.pdf", 
       plot = MANE_sPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
...
full_tpm = listgenes %>% rowwise() %>%  mutate(WTmean = mean(c_across(1:2)), KDmean = mean(c_across(3:4))) %>% ungroup()
...
write_csv(PTC_tpm, "SNRPC_PTC_MANE_TPM.csv")
write_csv(PTC_tpm, "C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/SNRPC_PTC_MANE_TPM.csv")
...
write_csv(WTalltrans,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/SNRPC_WTfilt_alltrans.csv")
write_csv(alltrans,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/SNRPC_DBfilt_alltrans.csv")
write_csv(alltransSPTC_MANE,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/SNRPC_PTC_alltrans.csv")
...
write_csv(WT5alltrans,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/SNRPC_WT5filt_alltrans.csv")
