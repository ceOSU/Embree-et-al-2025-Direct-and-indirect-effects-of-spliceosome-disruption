...
#Analyzing: Gene Level analysis of SF3A3 KD
...
PCA
ggsave("SF3A3_geneLevel_PCA.pdf",
       plot = PCA,
       device = pdf,
       width = 5,
       height = 5,
...
alltrans_NMD = alltransbio %>% mutate(NMD = case_when(ENSG.ID %in% SPTC_genes$geneID ~ "NMD",
                                                      ...
                                                      TRUE ~ NA))
write_csv(alltrans_NMD,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/SF3A3_geneLevel_NMD_alltrans.csv")
...
GL_NMD_box = GL_NMD_box + geom_boxplot(aes(x = NMD,
  ...
  labs(y = "log2(Fold Change)",
       ...
       title = "SF3A3 KD Gene Level Analysis")+
  coord_cartesian(y = c(-4,4)) +
  theme_bw()
GL_NMD_box
ggsave("SF3A3_geneLevel_NMD.pdf",
       plot = GL_NMD_box,
       device = pdf,
       width = 10,
       height = 10,
