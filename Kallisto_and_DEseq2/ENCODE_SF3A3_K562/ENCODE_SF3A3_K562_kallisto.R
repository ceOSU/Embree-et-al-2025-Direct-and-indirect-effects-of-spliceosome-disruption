...
#C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_SF3A3_K562 --> working directory of the project
...
setwd(dir = "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_SF3A3_K562/")

#Load File containing sample types

samples <- read_excel("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_SF3A3_K562/samples.xlsx")

View(samples)

#Load in the files

files1 <- file.path("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_SF3A3_K562","koutput", samples$Run, "abundance.h5")
...
SF3A3_K562transcripts=filter(listgenes, external_gene_name == "SF3A3" )

SF3A3_K562transcriptsWT<-SF3A3_K562transcripts %>% dplyr::select(WT1,WT2, ensembl_transcript_id_version, external_gene_name, external_transcript_name)

SF3A3_K562transcriptsWT$averagecounts = rowMeans(SF3A3_K562transcriptsWT[,c("WT1","WT2")])

#put in specific library names above

SF3A3_K562transcriptsWT$percent= SF3A3_K562transcriptsWT$averagecounts/sum(SF3A3_K562transcriptsWT$averagecounts)

SF3A3_K562transcriptsWT<-SF3A3_K562transcriptsWT %>% arrange(averagecounts)

#Repeat for the KD transcripts

SF3A3_K562transcriptsKD<-SF3A3_K562transcripts %>% dplyr::select(KD1,KD2, ensembl_transcript_id_version, external_gene_name, external_transcript_name)

SF3A3_K562transcriptsKD$averagecounts = rowMeans(SF3A3_K562transcriptsKD[,c("KD1","KD2")])

SF3A3_K562transcriptsKD$percent= SF3A3_K562transcriptsKD$averagecounts/sum(SF3A3_K562transcriptsKD$averagecounts)

SF3A3_K562transcriptsKD<-SF3A3_K562transcriptsKD %>% arrange(averagecounts)
...
plot(euler(s), quantities = TRUE,
     main="SF3A3 knockout vs control")
...
NMDBio_cdf = ggplot(alltransbioNMD, aes(log2FoldChange, colour=transcript_biotype))+
  ...
  labs(y = "Cumulative Frequency", title = "SF3A3 KD vs WT", subtitle = "NMD biotype")
NMDBio_cdf

#### analysis with robert's PTC+ and PTC- list ####
ENST_PTC.EPI.TFG <- read.delim("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_SF3A3_K562/ENST_PTC-EPI-TFG.txt")
...
PTC_cdf = ggplot(alltransPTC, aes(log2FoldChange, colour=PTC.Status))+
  ...
  coord_cartesian(xlim = c(-3,3)) +
  scale_color_manual(values = met.brewer("Java",2)) +
  labs(y = "Cumulative Frequency", title = "SF3A3 KD vs WT", subtitle = "PTC list")
PTC_cdf
...
NMD_transcripts = read.delim("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_SF3A3_K562/NMD_transcript_list.txt")
...
NMD_cdf = ggplot(alltransNMD, aes(log2FoldChange, colour=NMD))+
  ...
  coord_cartesian(xlim = c(-3,3)) +
  scale_color_manual(values = met.brewer("Java",2)) +
  labs(y = "Cumulative Frequency", title = "SF3A3 KD vs WT", subtitle = "NMD +/- list")
NMD_cdf
...
StringentNMD = read.csv("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_SF3A3_K562/Stringent_NMD_list_CE")
...
sNMD_cdf = ggplot(alltransSTNMD, aes(log2FoldChange, colour=NMD))+
  ...
  coord_cartesian(xlim = c(-3,3)) +
  scale_color_manual(values = met.brewer("Java",2)) +
  labs(y = "Cumulative Frequency", title = "SF3A3 KD vs WT", subtitle = "Stringent NMD list")
sNMD_cdf
...
sPTC = read.csv("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_SF3A3_K562/Stringent_PTC_CE") 
alltransSPTC = inner_join(alltrans, sPTC, by = c("ENST.ID" = "transID")) 
sPTC_res <- wilcox.test(log2FoldChange ~ PTC, data = alltransSPTC,
                        exact = FALSE, alternative = "less")
head(alltransSPTC)
count(alltransSPTC, PTC) #865 PTC false 270 PTC true
sPTC_res #p-vlaue = 2.898e-3
...
sPTC_cdf = sPTC_cdf + annotate("text", x=-2.5,y=1, label = "SF3A3 KD vs WT", size = 5, hjust = 0) +
  annotate("text", x=-2.5,y=.95, label = "Stringent PTC containing transcripts", hjust =0) +
  annotate("text", x=-2.5,y=.9, label = "p-value = 2.898e-3", hjust = 0) +
  annotate("text", x = -2.5, y = 0.85, label = "PTC+ = 270", hjust = 0) +
  annotate("text", x = -2.5, y = 0.8, label = "PTC- = 865", hjust = 0) +
  geom_vline(xintercept = 0, color = "grey") + 
  geom_hline(yintercept = 0.5, color = "grey") +
  theme(legend.position = c(0.85,0.1))
sPTC_cdf
sPTC_cdf
ggsave("SF3A3_K562_sPTC_CDF.pdf", 
       plot = sPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
       ...
       device = "pdf",
       dpi = 300)
ggsave("C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/SF3A3_K562_sPTC_CDF.pdf", 
       plot = sPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
...
SF3A3_x = alltransSTNMD %>% filter(NMD == "TRUE")  %>% #Makes a data table of the top NMD transcript for each gene
  select(Gene,log2FoldChange,baseMean) %>% 
  rename(plusFC = log2FoldChange) %>% 
  group_by(Gene) %>% 
  slice_max(baseMean, n = 1)

SF3A3_y  = alltransSTNMD %>% filter(NMD == "FALSE")  %>% #Makes a data table fo the top NMD- transcript for each gene
  select(Gene,log2FoldChange,baseMean) %>% 
  rename(minusFC = log2FoldChange) %>% 
  group_by(Gene) %>% 
  slice_max(baseMean, n = 1)

SF3A3_scatter = full_join(SF3A3_x, SF3A3_y, by = "Gene", suffix = c("plus","minus")) #Combine the two top tables
SF3A3_scatter = SF3A3_scatter %>% filter(!is.na(minusFC)) %>% 
  filter(!is.na(plusFC)) #Removes any NA rows


dot = ggplot(data = SF3A3_scatter)+
  ...
  labs(x = "Log2FC NMD+", y = "Log2FC NMD-", title = "SF3A3 gene-level FC comparison")+
  theme_minimal()+
...
SF3A3_NMD = alltrans_MANE %>% inner_join(NMD_factors, by = c("external_gene_name" = "NMDfactor"))
write.csv(SF3A3_NMD, "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/NMD_analysis/SF3A3_NMDfactor.csv",row.names = FALSE)

## CDF plot with the archive PE list
...
PEa_res <- wilcox.test(log2FoldChange ~ isoform, data = PEa_FC,
                       exact = FALSE, alternative = "less")
head(PEa_FC)
count(PEa_FC, isoform)# 308 PE  225 MANE
PEa_res #p-vlaue = 0.1.035e-4
...
PEa_cdf = ggplot(PEa_FC, aes(log2FoldChange, colour=isoform))+
  ...
  scale_color_manual(values = plot_colors) +
  labs(y = "Cumulative Frequency")
PEa_cdf = PEa_cdf + annotate("text", x=-2.5,y=1, label = "SF3A3 KD vs WT", size = 5, hjust = 0) +
  annotate("text", x=-2.5,y=.95, label = "Transcripts with a poison exon", hjust =0) +
  annotate("text", x=-2.5,y=.9, label = "p-value = 1.035e-4", hjust = 0) +
  annotate("text", x = -2.5, y = 0.85, label = "PE = 308", hjust = 0) +
  annotate("text", x = -2.5, y = 0.8, label = "MANE = 225", hjust = 0) +
  geom_vline(xintercept = 0, color = "grey") + 
  geom_hline(yintercept = 0.5, color = "grey") +
  theme(legend.position = c(0.85,0.1))
PEa_cdf
ggsave("SF3A3_K562_PE_archive_CDF.pdf", 
       plot = PEa_cdf,
       scale = 1,
       width = 8,
       height = 6,
       ...
       device = "pdf",
       dpi = 300)
ggsave("C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/SF3A3_K562_PEarchive_CDF.pdf", 
       plot = PEa_cdf,
       scale = 1,
       width = 8,
       height = 6,
...
SF3A3_SC_impact = alltrans_MANE %>% inner_join(Splicing_factors, by = c("external_gene_name" = "Component"))
write.csv(SF3A3_SC_impact, "SF3A3_splicing_impact.csv",row.names = FALSE)
...
sPTC_MANE_res <- wilcox.test(log2FoldChange ~ PTC, data = alltransSPTC_MANE,
                             exact = FALSE, alternative = "less")
count(alltransSPTC_MANE, PTC) #257 PTC false 270 PTC true
sPTC_MANE_res #p-vlaue = 1.02e-3
...
MANE_sPTC_cdf = ggplot(alltransSPTC_MANE, aes(log2FoldChange, colour=PTC))+
  ...
  scale_color_manual(values = plot_colors) +
  labs(y = "Cumulative Frequency")
MANE_sPTC_cdf = MANE_sPTC_cdf + annotate("text", x=-2.5,y=1, label = "SF3A3 KD vs WT", size = 5, hjust = 0) +
  annotate("text", x=-2.5,y=.95, label = "PTC containing and MANE transcripts", hjust =0) +
  annotate("text", x=-2.5,y=.9, label = "p-value = 1.02e-3", hjust = 0) +
  annotate("text", x = -2.5, y = 0.85, label = "PTC+ = 257", hjust = 0) +
  annotate("text", x = -2.5, y = 0.8, label = "MANE = 270", hjust = 0) +
  geom_vline(xintercept = 0, color = "grey") + 
  geom_hline(yintercept = 0.5, color = "grey") +
  theme(legend.position = c(0.85,0.1))
MANE_sPTC_cdf
ggsave("SF3A3_K562_sPTC_MANE_CDF.pdf", 
       plot = MANE_sPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
       ...
       device = "pdf",
       dpi = 300)
ggsave("C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/SF3A3_K562_sPTC_MANE_CDF.pdf", 
       plot = MANE_sPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
...
write_csv(PTC_tpm, "SF3A3_PTC_MANE_TPM.csv")
write_csv(PTC_tpm, "C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/SF3A3_PTC_MANE_TPM.csv")
...
write_csv(WTalltrans,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/SF3A3_WTfilt_alltrans.csv")
write_csv(alltrans,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/SF3A3_DBfilt_alltrans.csv")
write_csv(alltransSPTC_MANE,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/SF3A3_PTC_alltrans.csv")
...
write_csv(WT5alltrans,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/SF3A3_WT5filt_alltrans.csv")
