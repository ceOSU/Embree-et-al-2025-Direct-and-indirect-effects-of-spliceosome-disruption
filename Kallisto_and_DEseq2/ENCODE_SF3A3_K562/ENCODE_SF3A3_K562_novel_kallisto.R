...
novel_genes = counts %>% filter(str_detect(ENSTID,"MST")) %>% 
  left_join(SF3A3_novel_iso_features, by = c("ENSTID" = "isoform_id")) %>% 
  filter(!is.na(PTC))
...
write_csv(full_genelist, "C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/SF3A3_nk_TPM.csv")
