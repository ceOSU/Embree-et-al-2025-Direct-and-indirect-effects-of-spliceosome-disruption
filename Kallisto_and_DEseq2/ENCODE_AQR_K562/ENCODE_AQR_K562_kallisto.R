...
# C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_AQR_K562 --> working directory of the project
...
samples <- read_excel("AQR_K562_samples.xlsx")
View(samples)


#Load in the files
files1 <- file.path("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_AQR_K562","koutput", samples$Run, "abundance.h5")
files1
...
counts$mean=rowMeans(counts[,c("SRR14846211","SRR14846212")], na.rm=TRUE) #Filter WT samples
counts=filter(counts, mean>=1)
counts$mean=rowMeans(counts[,c("SRR14844828","SRR14844829")], na.rm=TRUE) #Fiter KD samples
counts=filter(counts, mean>=1)
counts$mean <- NULL
...
AQRtranscripts=filter(listgenes, external_gene_name == "AQR" )
AQRtranscriptsWT<-AQRtranscripts %>% dplyr::select(SRR14846211,SRR14846212, ensembl_transcript_id_version, external_gene_name, external_transcript_name)
AQRtranscriptsWT$averagecounts = rowMeans(AQRtranscriptsWT[,c("SRR14846211","SRR14846212")])
#put in specific library names above
AQRtranscriptsWT$percent= AQRtranscriptsWT$averagecounts/sum(AQRtranscriptsWT$averagecounts)
AQRtranscriptsWT<-AQRtranscriptsWT %>% arrange(averagecounts)

#Repeat for the KD transcripts
AQRtranscriptsKD<-AQRtranscripts %>% dplyr::select(SRR14844828,SRR14844829, ensembl_transcript_id_version, external_gene_name, external_transcript_name)
AQRtranscriptsKD$averagecounts = rowMeans(AQRtranscriptsKD[,c("SRR14844828","SRR14844829")])
AQRtranscriptsKD$percent= AQRtranscriptsKD$averagecounts/sum(AQRtranscriptsKD$averagecounts)
AQRtranscriptsKD<-AQRtranscriptsKD %>% arrange(averagecounts)
...
NMD_transcripts = read.delim("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_AQR_K562/NMD_transcript_list.txt")
...
NMD_cdf = ggplot(alltransNMD, aes(log2FoldChange, colour=NMD))+
  ...
  coord_cartesian(xlim = c(-3,3)) +
  scale_color_manual(values = met.brewer("Java",2)) +
  labs(y = "Cumulative Frequency", title = "AQR KD vs WT", subtitle = "NMD +/- list")
NMD_cdf
...
StringentNMD = read.csv("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_AQR_K562/Stringent_NMD_list_CE")
...
sNMD_cdf = ggplot(alltransSTNMD, aes(log2FoldChange, colour=NMD))+
  ...
  coord_cartesian(xlim = c(-3,3)) +
  scale_color_manual(values = met.brewer("Java",2)) +
  labs(y = "Cumulative Frequency", title = "AQR KD vs WT", subtitle = "Stringent NMD list")
sNMD_cdf
...
sPTC_res <- wilcox.test(log2FoldChange ~ PTC, data = alltransSPTC,
                        exact = FALSE, alternative = "less")
head(alltransSPTC)
count(alltransSPTC, PTC) #1200 PTC false 440 PTC true
sPTC_res #p-vlaue = 5.562e-12
...
sPTC_cdf = sPTC_cdf + annotate("text", x=-2.5,y=1, label = "AQR KD vs WT", size = 5, hjust = 0) +
  annotate("text", x=-2.5,y=.95, label = "Stringent PTC containing transcripts", hjust =0) +
  annotate("text", x=-2.5,y=.9, label = "p-value = 5.562e-12", hjust = 0) +
  annotate("text", x = -2.5, y = 0.85, label = "PTC+ = 440", hjust = 0) +
  annotate("text", x = -2.5, y = 0.8, label = "PTC- = 1200", hjust = 0) +
  geom_vline(xintercept = 0, color = "grey") + 
  geom_hline(yintercept = 0.5, color = "grey") +
  theme(legend.position = c(0.85,0.1))
sPTC_cdf
ggsave("AQR_K562_sPTC_CDF.pdf", 
       plot = sPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
       ...
       device = "pdf",
       dpi = 300)
ggsave("C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/AQR_K562_sPTC_CDF.pdf", 
       plot = sPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
...
write.csv(alltransSPTC, file = "AQR_sPTC")
testAQR = read.csv("AQR_sPTC")


##Make dot plot comparing the NMD plus and minus FC##
AQR_x = alltransSTNMD %>% filter(NMD == "TRUE")  %>% #Makes a data table of the top NMD transcript for each gene
  select(Gene,log2FoldChange,baseMean) %>% 
  rename(plusFC = log2FoldChange) %>% 
  group_by(Gene) %>% 
  slice_max(baseMean, n = 1)

AQR_y  = alltransSTNMD %>% filter(NMD == "FALSE")  %>% #Makes a data table fo the top NMD- transcript for each gene
  select(Gene,log2FoldChange,baseMean) %>% 
  rename(minusFC = log2FoldChange) %>% 
  group_by(Gene) %>% 
  slice_max(baseMean, n = 1)

AQR_scatter = full_join(AQR_x, AQR_y, by = "Gene", suffix = c("plus","minus")) #Combine the two top tables
AQR_scatter = AQR_scatter %>% filter(!is.na(minusFC)) %>% 
  filter(!is.na(plusFC)) #Removes any NA rows


dot = ggplot(data = AQR_scatter)+
  ...
  labs(x = "Log2FC NMD+", y = "Log2FC NMD-", title = "AQR gene-level FC comparison")+
  theme_minimal()+
...
#Remove Alternately spliced genes from the sPTC list
ASgenes_AQR <- read_delim("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/rMATS/ENCODE_AQR/ASgenes_AQR.csv", 
                          delim = ";", escape_double = FALSE, trim_ws = TRUE)
ASgenes_AQR = ASgenes_AQR %>% select(GeneID,geneSymbol)
...
sPTC_AS = sPTC_gene %>% anti_join(ASgenes_AQR, by = c("ensembl_gene_id" = "GeneID"))
count(sPTC_AS, PTC) #PTC false=215  PTC true= 37
alltransAS = inner_join(alltrans, sPTC_AS, by = c("ENST.ID" = "ensembl_transcript_id"))
count(alltransAS, PTC) # 109 PTC false  25 PTC true


asPTC_res <- wilcox.test(log2FoldChange ~ PTC, data = alltransAS,
                         exact = FALSE, alternative = "less")
head(alltransAS)
asPTC_res #p-vlaue = 0.8734
...
asPTC_cdf = ggplot(alltransAS, aes(log2FoldChange, colour=PTC))+
  ...
  coord_cartesian(xlim = c(-3,3)) +
  scale_color_manual(values = met.brewer("Java",2)) +
  labs(y = "Cumulative Frequency", title = "AQR KD vs WT", subtitle = "Non-AS transcripts")
asPTC_cdf = asPTC_cdf + annotate("text", x=-2.5,y=1, label = "p-value = 0.8734") +
  annotate("text", x = -2.5, y = 0.95, label = "PTC+ = 25") +
  annotate("text", x = -2.5, y = 0.9, label = "PTC- = 109") +
  geom_vline(xintercept = 0, color = "grey") + 
  geom_hline(yintercept = 0.5, color = "grey")
asPTC_cdf
ggsave("AQR_K562_asPTC_CDF.pdf", 
       plot = asPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
...
write.csv(AQR_NMD, "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/NMD_analysis/AQR_NMDfactor.csv",row.names = FALSE)
...
count(shared_SPTC_trans, PTC)#279 PTC false 101 PTC true
shared_sPTC_res #p-vlaue = 1e-4
...
shared_sPTC_cdf = ggplot(shared_SPTC_trans, aes(log2FoldChange, colour=PTC))+
  ...
  coord_cartesian(xlim = c(-3,3)) +
  scale_color_manual(values = met.brewer("Java",2)) +
  labs(y = "Cumulative Frequency", title = "AQR KD vs WT", subtitle = "Shared stringent PTC transcripts")
shared_sPTC_cdf = shared_sPTC_cdf + annotate("text", x=-2.5,y=1, label = "p-value = 1e-4") +
  annotate("text", x = -2.5, y = 0.95, label = "PTC+ = 101") +
  annotate("text", x = -2.5, y = 0.9, label = "PTC- = 279") +
  geom_vline(xintercept = 0, color = "grey") + 
  geom_hline(yintercept = 0.5, color = "grey")
shared_sPTC_cdf
ggsave("AQR_K562_shared_PTC_CDF.pdf", 
...
PE_res <- wilcox.test(log2FoldChange ~ isoform, data = PE_FC,
                      exact = FALSE, alternative = "less")
head(PE_FC)
count(PE_FC, isoform)# 87 PE  68 MANE
PE_res #p-vlaue = 3.962e-5
...
PE_cdf = ggplot(PE_FC, aes(log2FoldChange, colour=isoform))+
  ...
  scale_color_manual(values = plot_colors) +
  labs(y = "Cumulative Frequency")
PE_cdf = PE_cdf + annotate("text", x=-2.5,y=1, label = "AQR KD vs WT", size = 5, hjust = 0) +
  annotate("text", x=-2.5,y=.95, label = "Transcripts with a poison exon", hjust =0) +
  annotate("text", x=-2.5,y=.9, label = "p-value = 3.962e-5", hjust = 0) +
  annotate("text", x = -2.5, y = 0.85, label = "PE = 87", hjust = 0) +
  annotate("text", x = -2.5, y = 0.8, label = "MANE = 68", hjust = 0) +
  geom_vline(xintercept = 0, color = "grey") + 
  geom_hline(yintercept = 0.5, color = "grey") +
  theme(legend.position = c(0.85,0.1))
PE_cdf
ggsave("AQR_K562_PE_CDF.pdf", 
       plot = PE_cdf,
       scale = 1,
       width = 8,
       height = 6,
       ...
       device = "pdf",
       dpi = 300)
ggsave("C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/AQR_K562_PE_CDF.pdf", 
       plot = PE_cdf,
       scale = 1,
       width = 8,
       height = 6,
...
PEa_res <- wilcox.test(log2FoldChange ~ isoform, data = PEa_FC,
                       exact = FALSE, alternative = "less")
head(PEa_FC)
count(PEa_FC, isoform)# 463 PE  225 MANE
PEa_res #p-vlaue = <2.2e-16
...
PEa_cdf = ggplot(PEa_FC, aes(log2FoldChange, colour=isoform))+
  ...
  scale_color_manual(values = plot_colors) +
  labs(y = "Cumulative Frequency")
PEa_cdf = PEa_cdf + annotate("text", x=-2.5,y=1, label = "AQR KD vs WT", size = 5, hjust = 0) +
  annotate("text", x=-2.5,y=.95, label = "Transcripts with a poison exon", hjust =0) +
  annotate("text", x=-2.5,y=.9, label = "p-value = <2.2e-16", hjust = 0) +
  annotate("text", x = -2.5, y = 0.85, label = "PE = 463", hjust = 0) +
  annotate("text", x = -2.5, y = 0.8, label = "MANE = 225", hjust = 0) +
  geom_vline(xintercept = 0, color = "grey") + 
  geom_hline(yintercept = 0.5, color = "grey") +
  theme(legend.position = c(0.85,0.1))
PEa_cdf
ggsave("AQR_K562_PE_archive_CDF.pdf", 
       plot = PEa_cdf,
       scale = 1,
       width = 8,
       height = 6,
       ...
       device = "pdf",
       dpi = 300)
ggsave("C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/AQR_K562_PEarchive_CDF.pdf", 
       plot = PEa_cdf,
       scale = 1,
       width = 8,
       height = 6,
...
sPTC_MANE = read.csv("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_AQR_K562/Stringent_PTC_MANE_CE.csv")
...
sPTC_MANE_res <- wilcox.test(log2FoldChange ~ PTC, data = alltransSPTC_MANE,
                             exact = FALSE, alternative = "less")
count(alltransSPTC_MANE, PTC) # 259 PTC false 440 PTC true
...
MANE_sPTC_cdf = MANE_sPTC_cdf + annotate("text", x=-2.5,y=1, label = "AQR KD vs WT", size = 5, hjust = 0) +
  ...
  annotate("text", x=-2.5,y=.9, label = "p-value = <2.2e-16", hjust = 0) +
  annotate("text", x = -2.5, y = 0.85, label = "PTC+ = 440", hjust = 0) +
  annotate("text", x = -2.5, y = 0.8, label = "PTC- = 259", hjust = 0) +
  geom_vline(xintercept = 0, color = "grey") + 
  geom_hline(yintercept = 0.5, color = "grey") +
  theme(legend.position = c(0.85,0.1))
MANE_sPTC_cdf
...
write_csv(PTC_tpm, "C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/AQR_PTC_MANE_TPM.csv")

#### Look at the effect when only WT filtering####
WTcounts <-as.data.frame(txi.kallisto.tsv$counts)

#Filtering out transcripts with mean tpm<1
WTcounts$mean=rowMeans(WTcounts[,c("SRR14846211","SRR14846212")], na.rm=TRUE) #Filter WT samples
...
write_csv(WTalltrans,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/AQR_WTfilt_alltrans.csv")
write_csv(alltrans,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/AQR_DBfilt_alltrans.csv")
write_csv(alltransSPTC_MANE,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/AQR_PTC_alltrans.csv")
...
WT5counts$mean=rowMeans(WT5counts[,c("SRR14846211","SRR14846212")], na.rm=TRUE) #Filter WT samples
...
write_csv(WT5alltrans,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/AQR_WT5filt_alltrans.csv")
