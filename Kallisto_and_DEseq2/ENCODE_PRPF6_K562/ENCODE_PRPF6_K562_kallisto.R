...
#Modified on: 5/20/2024
#Analyzing: PRPF6 KD from ENCODE
...
# C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_PRPF6 --> working directory of the project
# SRR4422262 --> Sample ID of first KD sample
# SRR4422263 --> Sample ID of second KD sample
# SRR4422353 --> Sample ID of first WT sample
# SRR4422354 --> Sample ID of second WT sample
# PRPF6 --> Ensembl name of your gene of interest
...
#### Load in the files ####
files1 <- file.path("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_PRPF6","koutput", samples$Run, "abundance.h5")
names(files1) <- paste0(c("SRR4422262","SRR4422263","SRR4422353","SRR4422354"))
files1
...
counts$mean=rowMeans(counts[,c("SRR4422262","SRR4422263")], na.rm=TRUE) #Filter KD samples
counts=filter(counts, mean>=1)
counts$mean=rowMeans(counts[,c("SRR4422353","SRR4422354")], na.rm=TRUE) #Fiter WT samples
counts=filter(counts, mean>=1)
counts$mean <- NULL
...
plotMA(res.sig)
sum( res.sig$log2FoldChange < 0, na.rm=TRUE )#upregulated genes=378
sum( res.sig$log2FoldChange > 0, na.rm=TRUE )#downregulated genes=263
...
#log2fc range : +8.3128 to -7.6358

#Exporting files
write.csv(as.data.frame(res_counts), file = "KDvsWT_full_results.csv")
write.csv(as.data.frame(res.sig), file = "KDvsWT_sig_results.csv")
...
plot(euler(s), quantities = TRUE,
     main="PRPF6 knockout vs control")
...
NMDBio_cdf = ggplot(alltransbioNMD, aes(log2FoldChange, colour=transcript_biotype))+
  ...
  labs(y = "Cumulative Frequency", title = "PRPF6 KD vs WT", subtitle = "NMD biotype")
NMDBio_cdf

#### analysis with robert's PTC+ and PTC- list ####
ENST_PTC.EPI.TFG <- read.delim("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/ENCODE_PRPF6/ENST_PTC-EPI-TFG.txt")
...
PTC_cdf = ggplot(alltransPTC, aes(log2FoldChange, colour=PTC.Status))+
  ...
  coord_cartesian(xlim = c(-3,3)) +
  scale_color_manual(values = met.brewer("Java",2)) +
  labs(y = "Cumulative Frequency", title = "PRPF6 KD vs WT", subtitle = "PTC list")
PTC_cdf
...
sPTC_MANE_res <- wilcox.test(log2FoldChange ~ PTC, data = alltransSPTC_MANE,
                             exact = FALSE, alternative = "less")
count(alltransSPTC_MANE, PTC) # 247 PTC false 190 PTC true
sPTC_MANE_res #p-vlaue = 0.8948
...
MANE_sPTC_cdf = MANE_sPTC_cdf + annotate("text", x=-2.5,y=1, label = "PRPF6 KD vs WT", size = 5, hjust = 0) +
  annotate("text", x=-2.5,y=.95, label = "PTC containing and MANE transcripts", hjust =0) +
  annotate("text", x=-2.5,y=.9, label = "p-value = 0.8948", hjust = 0) +
  annotate("text", x = -2.5, y = 0.85, label = "PTC+ = 190", hjust = 0) +
  annotate("text", x = -2.5, y = 0.8, label = "MANE = 247", hjust = 0) +
  geom_vline(xintercept = 0, color = "grey") + 
  geom_hline(yintercept = 0.5, color = "grey") +
  theme(legend.position = c(0.85,0.1))
MANE_sPTC_cdf
ggsave("PRPF6_K562_sPTC_MANE_CDF.pdf", 
       plot = MANE_sPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
       ...
       device = "pdf",
       dpi = 300)
ggsave("C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/PRPF6_K562_sPTC_MANE_CDF.pdf", 
       plot = MANE_sPTC_cdf,
       scale = 1,
       width = 8,
       height = 6,
...
PRPF6_NMD = alltrans_MANE %>% inner_join(NMD_factors, by = c("external_gene_name" = "NMDfactor"))
write.csv(PRPF6_NMD, "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/NMD_analysis/PRPF6_NMDfactor.csv",row.names = FALSE)
...
PRPF6_SC_impact = alltrans_MANE %>% inner_join(Splicing_factors, by = c("external_gene_name" = "Component"))
write.csv(PRPF6_SC_impact, "PRPF6_splicing_impact.csv",row.names = FALSE)
...
full_tpm = full_tpm %>% select(8:11)
PTC_tpm = full_tpm %>% right_join(sPTC_MANE, by = c("ENST.ID" = "transID"))
PTC_tpm = PTC_tpm %>% select(2:4,5:6)
write_csv(PTC_tpm, "PRPF6_PTC_MANE_TPM.csv")
write_csv(PTC_tpm, "C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/PRPF6_PTC_MANE_TPM.csv")

#### Look at the effect when only WT filtering####
WTcounts <-as.data.frame(txi.kallisto.tsv$counts)

#Filtering out transcripts with mean tpm<1
WTcounts$mean=rowMeans(WTcounts[,c("SRR4422262","SRR4422263")], na.rm=TRUE) #Filter WT samples
...
write_csv(WTalltrans,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/PRPF6_WTfilt_alltrans.csv")
write_csv(alltrans,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/PRPF6_DBfilt_alltrans.csv")
write_csv(alltransSPTC_MANE,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/PRPF6_PTC_alltrans.csv")
...
WT5counts$mean=rowMeans(WT5counts[,c("SRR4422262","SRR4422263")], na.rm=TRUE) #Filter WT samples
...
write_csv(WT5alltrans,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/PRPF6_WT5filt_alltrans.csv")
