 ...
 **Code chunks that do not display output have been run, but have had results turned off.**
 ...
 8/6/2024: Added in analysis to look at PE vs all cassette exons
...
```{r Look at PSI of PE}
Saltzman_transcripts <- read_csv("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/Poison_Exons/Saltzman_inclusion_NMD_transcripts.csv") #Import poison exon coordinates
Saltzman_inclusion_exons <- read_csv("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/Poison_Exons/Saltzman_inclusion_exons.csv")
all_cassette_exons <- read_delim("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/Poison_Exons/all_cassette_exons.txt", 
    delim = "\t", escape_double = FALSE, 
    trim_ws = TRUE)
all_cassette_exons = all_cassette_exons %>%
  filter(genename != "onlyEST") %>% #Removes the exons that don't have a gene name associated
  select(1:4,22) #Exon start coordinates are already 0 base
cassette_annotation = all_cassette_exons %>% mutate(PE = if_else(genename %in% Saltzman_transcripts$external_gene_name,
                                                                 TRUE,FALSE))

all_cassette_inc = tibble(Sample = character())
for (i in all_order) {
  print(i)
  assign(paste0(i,"_cassettes"), eval(parse(text = paste0(i,"_SE_JCEC"))) %>%
           full_join(eval(parse(text = paste0(i,"_MXE_JCEC")))) %>% #combine the MXE and SE tables
           inner_join(cassette_annotation, by = c("geneSymbol" = "genename",
                                                  "chr" = "chromo",
                                                  "strand",
                                                  "exonStart_0base" = "start",
                                                  "exonEnd" = "end"),
                      relationship = "many-to-one"))
  assign(paste0(i,"_cassettes"), eval(parse(text = paste0(i,"_cassettes"))) %>%
           mutate(Sample = i))
  all_cassette_inc = all_cassette_inc %>% full_join(eval(parse(text = paste0(i,"_cassettes"))))
}

all_cassette_inc = all_cassette_inc %>% mutate(corec_ILD = IncLevelDifference * -1)
all_cassette_sum = all_cassette_inc %>% group_by(Sample,PE) %>% summarise(n = n(),
                                                                       Med = median(corec_ILD))
all_PE_res = tibble(Sample = as.character(), P = as.numeric())
for (i in all_order) {
  print(i)
  assign(paste0(i,"_PE_res"), wilcox.test(corec_ILD ~ PE, data = all_cassette_inc %>% filter(Sample == i),
                                          exact = FALSE, alternative = "less")) #use less because we expect other cassette exons to be lower included than PE
  all_PE_res = all_PE_res %>% add_row(Sample = i, P = eval(parse(text = paste0(i,"_PE_res$p.value"))))
}
all_cassette_sum = all_cassette_sum %>% left_join(all_PE_res)

select_list = c("UPF1","EIF4A3","AQR","SF3B1","SF3B3","CDC40","SNRPC")
PE_Colors = c("TRUE" = "#76CEB5","FALSE" = "grey")
PE_boxplot = ggplot(data = all_cassette_inc)
PE_boxplot = PE_boxplot + 
  geom_boxplot(aes(x = factor(Sample, levels = all_order),
                   y = corec_ILD,
                   fill = PE),
               ...
               outlier.colour = NA,
               linewidth = 1) +
  scale_fill_manual(values = PE_Colors,labels = c("TRUE" = "Poison",
                                                  "FALSE" = "Cassette")) +
  scale_color_manual(values = PE_Colors)+
  geom_label(data = all_cassette_sum,
             aes(x = factor(Sample, levels = all_order),
                 y = 0.75,
                 label = signif(P,digits = 3)),
             size = 5,
             alpha = 0.75)+
  geom_label(data = all_cassette_sum %>% filter(Sample %in% all_order),
             aes(x = factor(Sample, levels = all_order),
                 y = Med,
                 color = PE,
  ...
  labs(x = "Sample",
       y = "▲PE Inclusion",
       fill = "Exon Type",
       title = "Cassette exon inclusion")+
  coord_cartesian(y = c(-1,1))
PE_boxplot
ggsave("PE_boxplot_Full.pdf",
       plot = PE_boxplot,
       width = 40,
       height = 10,
       units = "in",
...
#Filter to select genes
select_PE_boxplot = ggplot(data = all_cassette_inc %>% filter(Sample %in% select_list))
select_PE_boxplot = select_PE_boxplot + 
  geom_boxplot(aes(x = factor(Sample, levels = all_order),
                   y = corec_ILD,
                   fill = PE),
               position = position_dodge2(width = 0.9),
  ...
  scale_fill_manual(values = PE_Colors,labels = c("TRUE" = "Poison",
                                                  "FALSE" = "Cassette")) +
  scale_color_manual(values = PE_Colors)+
  geom_label(data = all_cassette_sum %>% filter(Sample %in% select_list),
             aes(x = factor(Sample, levels = all_order),
                 y = 0.75,
                 label = signif(P,digits = 3)),
             size = 5,
  ...
  coord_cartesian(y = c(-1,1))
select_PE_boxplot
ggsave("select_PE_boxplot_Full.pdf",
       plot = select_PE_boxplot,
       width = 20,
       height = 10,
       units = "in",
       dpi = 300,
...
I'm also going to filter the results to just the significant altered splicing events. 

```{r}
sig_cassette = all_cassette_inc %>% filter(PValue < 0.05)
sig_cassette_sum = sig_cassette %>% group_by(Sample, PE) %>% 
  summarise(n = n(),
            Med = median(corec_ILD))
sig_cassette_res = tibble(Sample = as.character(), P = as.numeric())
for (i in all_order) {
  print(i)
  assign(paste0(i,"_sig_PE_res"), wilcox.test(corec_ILD ~ PE, data = sig_cassette %>% filter(Sample == i),
                                          exact = FALSE, alternative = "less")) #use less because we expect other cassette exons to be lower included than PE
  sig_cassette_res = sig_cassette_res %>% add_row(Sample = i, P = eval(parse(text = paste0(i,"_sig_PE_res$p.value"))))
}
sig_cassette_res = sig_cassette_res %>% mutate(padj = p.adjust(P,method = "BH"))
sig_cassette_sum = sig_cassette_sum %>% left_join(sig_cassette_res)


sig_PE_boxplot = ggplot(data = sig_cassette)
sig_PE_boxplot = sig_PE_boxplot + 
  geom_boxplot(aes(x = factor(Sample, levels = all_order),
                   y = corec_ILD,
                   fill = PE),
               position = position_dodge2(width = 0.9),
  ...
  scale_fill_manual(values = PE_Colors,labels = c("TRUE" = "Poison",
                                                  "FALSE" = "Cassette")) +
  scale_color_manual(values = PE_Colors)+
  geom_label(data = sig_cassette_sum,
             aes(x = factor(Sample, levels = all_order),
                 y = 0.75,
                 label = signif(padj,digits = 3)),
             size = 5,
             alpha = 0.75)+
  geom_label(data = sig_cassette_sum %>% filter(Sample %in% all_order),
             aes(x = factor(Sample, levels = all_order),
                 y = Med,
                 color = PE,
                 label = round(Med, digits = 3)),
...
#Filter to select genes
select_sig_PE_boxplot = ggplot(data = sig_cassette %>% filter(Sample %in% select_list))
select_sig_PE_boxplot = select_sig_PE_boxplot + 
  geom_boxplot(aes(x = factor(Sample, levels = all_order),
                   y = corec_ILD,
                   fill = PE),
               position = position_dodge2(width = 0.9),
  ...
  scale_fill_manual(values = PE_Colors,labels = c("TRUE" = "Poison",
                                                  "FALSE" = "Cassette")) +
  scale_color_manual(values = PE_Colors)+
  geom_label(data = sig_cassette_sum %>% filter(Sample %in% select_list),
             aes(x = factor(Sample, levels = all_order),
                 y = 0.75,
                 label = signif(padj,digits = 3)),
             size = 5,
  ...
  labs(x = "Sample",
       ...
       title = "Cassette exon inclusion")+
  coord_cartesian(y = c(-1,1))
select_sig_PE_boxplot
ggsave("select_sig_PE_boxplot.pdf",
       plot = select_sig_PE_boxplot,
       width = 20,
       height = 10,
       units = "in",
       dpi = 300,
...
I'm going to filter to just the poison exons that intorduce NMD upon inclusion of the exons.
With these I'll plot the change in percent spliced in that rMATS provides, which will do better than trying to define casette exons to match to. 
The change in PSI is calculated WT-KD, meaning that a negative value means the exon is more included in the knockdown.
I'm going to multiply the PSI by -1 so that it makes more sense graphically.

```{r}
select_SE_events = tibble(Sample = as.character())
for (i in select_list) {
  print(i)
  assign(paste0(i,"_SE_JCEC"),
         eval(parse(text = paste0(i,"_SE_JCEC"))) %>% 
           mutate(Sample = i))
  select_SE_events = select_SE_events %>% full_join(eval(parse(text = paste0(i,"_SE_JCEC"))))
}
PE_inclusion_exons = Saltzman_inclusion_exons %>% 
  mutate(chr = paste0("chr",chromosome_name),
         start = exon_chrom_start-1,
         end = exon_chrom_end-1) %>% 
  dplyr::select(ensembl_gene_id,ensembl_exon_id,start,end,chr)
PE_SE_events = select_SE_events %>% inner_join(PE_inclusion_exons,by = c("exonStart_0base" = "start",
                                                                         "exonEnd" = "end")) %>% 
  mutate(Match = if_else(GeneID == ensembl_gene_id,TRUE,FALSE),
         corILD = IncLevelDifference * -1)
PE_SE_events %>% group_by(Sample) %>% summarise(events = n(),genes = n_distinct(GeneID),exons = n_distinct(ensembl_exon_id))

PE_SE_plot = ggplot(data = PE_SE_events %>% filter(Match == TRUE)) +
  ...
  geom_boxplot(aes(x = Sample,
               ...
               alpha = 0.6,
               outlier.shape = NA) +
  scale_fill_moma_d("VanGogh") +
  theme_bw() +
  labs(x = "Sample",
       y = "▲PE Inclusion",
       title = "PE inclusion by coord") +
  coord_cartesian(y = c(-1,1))
PE_SE_plot
ggsave("PE_exon_coord_boxplot.pdf",
       plot = PE_SE_plot,
       width = 20,
       height = 10,
       units = "in",
...
select_PE <- read_excel("C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Revision - RNA Biology 2025/select_PE.xlsx")


EIF4A2_sig = select_SE_events %>% filter(geneSymbol == "EIF4A2") %>% filter(FDR < 0.05)
EIF4A2_sum = EIF4A2_sig %>% group_by(Sample) %>% summarise(n = n(),med = median(IncLevelDifference))
EIF4A2_plot = ggplot(EIF4A2_sig) +
  geom_boxplot(aes(x = factor(Sample,levels = all_order),
                   y = IncLevelDifference)) +
  geom_text(data = EIF4A2_sum,
            aes(x = factor(Sample,levels = all_order),
                y = -1,
                label = n)) +
  geom_label(data = EIF4A2_sum,
             aes(x = factor(Sample,levels = all_order),
                 y = med,
                 label = round(med,digits = 2))) +
  theme_bw() +
  coord_cartesian(ylim = c(-1,1))
EIF4A2_plot
ggsave("EIF4A2_SE_ILD.pdf",
       plot = EIF4A2_plot,
       device = pdf,
       width = 10,
       height = 10,
...
#Try using Bradley PE coordinates
thomas_S4_cleaned <- read_csv("C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Revision - RNA Biology 2025/thomas_S4_cleaned.csv")
thomas_S4_cleaned = thomas_S4_cleaned %>% dplyr::select(geneName,chr...7,chromStart_38) %>%
  rename(chr = chr...7) %>% 
  mutate(PE_start = chromStart_38 -1)
thomas_PE_events = select_SE_events %>% inner_join(thomas_S4_cleaned,by = c("exonStart_0base" = "PE_start"))
thomas_PE_events = thomas_PE_events %>% mutate(cor_ILD = IncLevelDifference * -1) %>% filter(FDR < 0.05)
end_coords = getBM(attributes = c("ensembl_gene_id","ensembl_transcript_id","transcript_biotype",
                                  "ensembl_exon_id","exon_chrom_start","exon_chrom_end"),
                   filters = "ensembl_gene_id",
                   values = thomas_PE_events$GeneID,
                   mart = hmart)
end_coords = end_coords %>% filter(transcript_biotype == "nonsense_mediated_decay") %>%
  mutate(exon_start = exon_chrom_start -1)
thomas_PE_events = thomas_PE_events %>% left_join(end_coords,by = c("GeneID" = "ensembl_gene_id",
                                                                    "exonStart_0base" = "exon_start"),
                                                  relationship = "many-to-many") %>% 
  mutate(match = if_else(exonEnd == exon_chrom_end,TRUE,FALSE))
thomas_PE_events = thomas_PE_events %>% filter(match == TRUE)
thomas_sum = thomas_PE_events %>% group_by(Sample) %>% summarise(n = n(),med = median(cor_ILD))

thomas_plot = ggplot(data = thomas_PE_events) +
  geom_boxplot(aes(x = factor(Sample, levels = all_order),
                   y = cor_ILD,
                   fill = Sample),
               outlier.shape = 21) +
  theme_bw() +
  geom_label(data = thomas_sum,
            aes(x = factor(Sample, levels = all_order),
                y = -0.5,
                color = Sample,
                label = n),
  ...
  labs(x = "Sample",
       y = "Delta PSI")
thomas_plot
ggsave("Thomas_PE_inclusion.pdf",
       plot = thomas_plot,
       device = pdf,
       width = 7,
       height = 5,
...
hmart =  useMart("ensembl",host = "https://feb2023.archive.ensembl.org") #use version 109 of ensembl
hmart <- useDataset("hsapiens_gene_ensembl",mart=hmart)
attributes <- listAttributes(hmart)
...
Complex_colors = c("Catalytic" = "#1c998b","Early" = "#fe5f55","NA" = "grey","NMD" = "black")
WT5_level = WT5_sum_genelevel %>% filter(sig_SE == "TRUE") %>% 
  select(prop,Sample) %>% 
  arrange(prop)
WT5_genelevel_prop = ggplot(data = WT5_sum_genelevel %>% filter(sig_SE == "TRUE"))
WT5_genelevel_prop = WT5_genelevel_prop + geom_point(aes(x = factor(Sample, levels = WT5_level$Sample),
                                                       y = prop,
                                                       color = Splice_Stage),
  ...
  labs(x = element_blank(),
       y = "Fraction of Genes",
       title = "Fraction of highly expressed genes undergoing AS",
       fill = "Spliceosome Stage")
WT5_genelevel_prop
...
all_events_summary = all_events %>% group_by(Sample) %>% 
  summarise(total_ann = sum(TotalEventsJCEC),
            Significant = sum(SignificantEventsJCEC),
            Novel = sum(NovelEvents)) %>% 
  left_join(Sample_complex,by = "Sample") %>% 
  left_join(read_depth,by = "Sample") %>% 
...
all_event_level = all_events_summary %>% filter(Type == "Sig_norm") %>% 
  select(Number,Sample) %>% 
  arrange(Number)
all_events_plot = ggplot(data = all_events_summary)
all_events_plot = all_events_plot + geom_point(aes(x = factor(Sample, levels = all_event_level$Sample),
                                                   y = log10(Number),
                                                   shape = factor(Type,
...
# Examination of splice altering drugs

I am going to look at the splicing changes that occur after treatment with drugs that effect the spliceosome.
Pladienolide B, THZ531, and Indusilam are spliceosome inhibitors while Risdiplam alters splice site usage. 

## Load in all the data
```{r}
SI_samples = c("pladienolide_B","THZ531","Indisulam","Risdiplam")
all_SI_sig = tibble(Sample = character())
all_SI_novel = tibble(Sample = character())
for (i in SI_samples) {
  print(i)
  ...
  write_csv(eval(parse(text = paste0(i,"_sig_combined"))),
            file = paste0("C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/",
                          i,
                          "_annotated_AS.csv"))
  all_SI_sig = all_SI_sig %>% full_join(eval(parse(text = paste0(i,"_sig_combined")))) %>% 
    mutate(Event_type = "Significant")
  ...
  assign(paste0(i,"_novel_events"),
         eval(parse(text = paste0(i,"_novel_SE"))) %>% 
           ...
           full_join(eval(parse(text = paste0(i,"_novel_RI")))) %>% 
           full_join(eval(parse(text = paste0(i,"_novel_MXE")))))
  all_SI_novel = all_SI_novel %>% full_join(eval(parse(text = paste0(i,"_novel_events")))) %>% 
    mutate(Event_type = "Novel")
}
all_SI_events = all_SI_novel %>% full_join(all_SI_sig)
write_csv(all_SI_events,"C:/Users/Caleb/OneDrive - The Ohio State University/Splicing and NMD/Figures/Data/NMD_TPM/disease_all_splicing_events")
```


## Make the event type plot
```{r}
#Novel proportions
SI_Novel_Event_sum = all_SI_novel %>% group_by(Sample,Event) %>% count()
SI_novel_prop_sum = SI_Novel_Event_sum %>% group_by(Sample) %>% mutate(prop = n/sum(n))
SI_novel_stacked = ggplot(data = SI_novel_prop_sum)
SI_novel_stacked = SI_novel_stacked + geom_col(aes(x = factor(Sample, levels = SI_samples),
                                             y = prop,
                                             fill = Event))
SI_novel_stacked = SI_novel_stacked + theme_minimal() +
  scale_fill_manual(values = met.brewer("Derain", n = 5)) +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        axis.line = element_line(color = "black")) +
  labs(y = "KD",
       ...
       caption = "Calculated using JCEC")
SI_novel_stacked
ggsave("SI_Novel_stacked_JCEC.pdf",
       plot = SI_novel_stacked,
       device = "pdf",
       scale = 1,
       width = 14,
       height = 10,
...
#Annotated proportions
SI_percent_sum = all_SI_sig %>% group_by(Sample,Type) %>% 
  count()
SI_percent_sum = SI_percent_sum %>% group_by(Sample) %>% 
  mutate(prop = n/sum(n))

SI_sig_stacked = ggplot(data = SI_percent_sum)
SI_sig_stacked = SI_sig_stacked + geom_col(aes(x = factor(Sample,levels = SI_samples), 
                                         y = prop, 
                                         fill = Type)) +
  theme_minimal() +
  scale_fill_manual(values = met.brewer("Derain", n = 5)) +
  theme(panel.grid.major = element_blank(),
        ...
        axis.line = element_line(color = "black")) +
  labs(x = "KD",
       y = "Proportion of Significant Splicing Changes",
       title = "Changes in Annotated Splicing Events Upon KD",
       caption = "Calculated using JCEC")
SI_sig_stacked
ggsave("SI_Sig_stacked_JCEC.pdf",
       plot = SI_sig_stacked,
       device = "pdf",
       scale = 1,
       width = 14,
...
```{r}
Saltzman_SC = getBM(attributes = c("ensembl_transcript_id","3_utr_end","end_position","transcript_end",
                                   "cds_end","strand"),
                    filters = "ensembl_transcript_id",
                    values = Saltzman_inclusion_exons$ensembl_transcript_id,
                    mart = hmart)
Saltzman_inclusion_exons = Saltzman_inclusion_exons %>% left_join(Saltzman_SC)
```
