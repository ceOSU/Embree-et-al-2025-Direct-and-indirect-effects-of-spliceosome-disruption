...
annotatedTPM = rawTPM %>% mutate(wtTPM = rowMeans(dplyr::select(rawTPM,2:3), na.rm = TRUE)) %>% #Make sure these refer to the correct columns
...
all_novel_tpm_filt = all_novel_tpm_filt %>% dplyr::select(3:12,18:19) %>% mutate(sample = "UPF1")
