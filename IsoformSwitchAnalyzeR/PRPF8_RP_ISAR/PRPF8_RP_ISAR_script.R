...
#### Copy and Paste ####
#PRPF8_RP = Gene of Interest
#wdPATH = Path to the working directory
#WT1 = First WT sample
#WT2 = Second WT sample
#KD1 = First KD sample
#KD2 = Second KD sample
...
library(tidyverse)
library(biomaRt)
setwd("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/PRPF8_RP_ISAR")

####Import datasets####
PRPF8_RPquant = importIsoformExpression(parentDir = "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/PRPF8_RP_ISAR",
                                   addIsofomIdAsColumn = TRUE, 
                                   showProgress = TRUE,
                                   readLength = 50
) #reads the t_data.ctab file in each sub directory
names(PRPF8_RPquant)
tail(PRPF8_RPquant$abundance)
tail(PRPF8_RPquant$counts)

#Generate the list of isoform switches
myDesign = data.frame(sampleID = c("SRR13507999","SRR13508000","SRR13508001",
                                   "SRR13507993","SRR13507994","SRR13507995"),
                      condition = c("RP","RP","RP","WT","WT","RP"))
comparisons = data.frame(condition_1 = "WT",
                         condition_2 = "RP")
SwitchList = importRdata(isoformCountMatrix = PRPF8_RPquant$counts,
                         isoformRepExpression = PRPF8_RPquant$abundance,
                         designMatrix = myDesign,
                         isoformExonAnnoation = "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/PRPF8_RP_ISAR/st_merged.gtf",
                         isoformNtFasta = "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/PRPF8_RP_ISAR/PRPF8_RP_transcripts.fa",
                         showProgress = TRUE,
...
rawTPM = PRPF8_RPquant$abundance
annotatedTPM = rawTPM %>% mutate(kdTPM = rowMeans(dplyr::select(rawTPM,5:7), na.rm = TRUE)) %>% #Make sure these refer to the correct columns
  mutate(wtTPM = rowMeans(dplyr::select(rawTPM, 2:4), na.rm = TRUE))
...
Novel_TPM = Novel_TPM %>% dplyr::select(1:9,11:16) %>%
  mutate(type = "Novel")
PTC_TPM = PTC_TPM %>% dplyr::select(1:9,11:16) %>%
  mutate(type = "PTC")
PTC_MANE_TPM = MANE_TPM %>% inner_join(Stringent_PTC, by = c("isoform_id" = "transID"))
PTC_MANE_TPM = PTC_MANE_TPM %>% dplyr::select(1:9,11:16) %>%
  mutate(type = "MANE")
PRPF8_RP_TPM = PTC_TPM %>% full_join(Novel_TPM) %>% full_join(PTC_MANE_TPM)
write_csv(PRPF8_RP_TPM, "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/Combined_analysis/PRPF8_RP_TPM.csv")
...
all_novel_tpm = all_novel_tpm %>% mutate(RPTPM = rowMeans(dplyr::select(all_novel_tpm,14:15), na.rm = TRUE))
...
all_novel_tpm_filt = all_novel_tpm_filt %>% dplyr::select(3:12,18:19) %>% mutate(sample = "PRPF8_RP")
write_csv(all_novel_tpm_filt, "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/Combined_analysis/PRPF8_RP_novel_TPM.csv")
...
length_plot
ggsave("PRPF8_RP_ISAR_lengths.pdf",
       plot = length_plot,
       height = 10,
       width = 10,
       units = "in",
       device = pdf,
       dpi = 300)
write_csv(switching_ORF, "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/Combined_analysis/PRPF8_RP_switch_ORF_length.csv")