...
#### Copy and Paste ####
#SNRPC = Gene of Interest
#C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/ENCODE_SNRPC_ISAR = Path to the working directory
#SRR14838740 = First WT sample
#SRR14838741 = Second WT sample
#SRR14845144 = First KD sample
#SRR14845145 = Second KD sample
...
library(dplyr)
library(biomaRt)
setwd("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/ENCODE_SNRPC_ISAR")

#Import datasets
SNRPCquant = importIsoformExpression(parentDir = "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/ENCODE_SNRPC_ISAR",
                                      addIsofomIdAsColumn = TRUE, 
                                      showProgress = TRUE,
                                      readLength = 100
) #reads the t_data.ctab file in each sub directory
names(SNRPCquant)
tail(SNRPCquant$abundance)
tail(SNRPCquant$counts)

#Generate the list of isoform switches
myDesign = data.frame(sampleID = c("SRR14838740","SRR14838741","SRR14845144","SRR14845145"),
                      condition = c("WT","WT","KD","KD"))
comparisons = data.frame(condition_1 = "WT",
                         condition_2 = "KD")
SwitchList = importRdata(isoformCountMatrix = SNRPCquant$counts,
                         isoformRepExpression = SNRPCquant$abundance,
                         designMatrix = myDesign,
                         isoformExonAnnoation = "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/ENCODE_SNRPC_ISAR/st_merged.gtf",
                         isoformNtFasta = "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/ENCODE_SNRPC_ISAR/SNRPC_transcripts.fa",
                         showProgress = TRUE,
...
PTC_MANE_TPM = PTC_MANE_TPM %>% dplyr::select(1:11,14) %>%
  mutate(type = "MANE")
SNRPC_TPM = PTC_TPM %>% full_join(Novel_TPM) %>% full_join(PTC_MANE_TPM)
write_csv(SNRPC_TPM, "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/Combined_analysis/SNRPC_TPM.csv")
...
all_novel_tpm_filt = all_novel_tpm_filt %>% dplyr::select(3:12,18:19) %>% mutate(sample = "SNRPC")
write_csv(all_novel_tpm_filt, "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/Combined_analysis/SNRPC_novel_TPM.csv")
...
ggsave("SNRPC_ISAR_lengths.pdf",
       ...
       device = pdf,
       dpi = 300)
write_csv(switching_ORF, "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/Combined_analysis/SNRPC_switch_ORF_length.csv")