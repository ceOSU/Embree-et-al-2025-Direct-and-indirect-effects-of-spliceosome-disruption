...
# Analyzing: PRPF4 KD from ENCODE with the runs from the same KD biological replicate concatenated together
...
#### Copy and Paste ####
#PRPF4 = Gene of Interest
#C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/PRPF4_merged_ISAR = Path to the working directory
#SRR14837933 = First WT sample
#SRR14837934 = Second WT sample
#PRPF4kd1 = First KD sample
#PRPF4kd2 = Second KD sample
...
library(rhdf5)
library(dplyr)
setwd("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/PRPF4_merged_ISAR")

#Import datasets
PRPF4quant = importIsoformExpression(parentDir = "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/PRPF4_merged_ISAR",
                                      addIsofomIdAsColumn = TRUE, 
                                      showProgress = TRUE,
                                      readLength = 100
) #reads the t_data.ctab file in each sub directory
names(PRPF4quant)
tail(PRPF4quant$abundance)
tail(PRPF4quant$counts)

#Generate the list of isoform switches
myDesign = data.frame(sampleID = c("SRR14837933","SRR14837934","PRPF4kd1","PRPF4kd2"),
...
SwitchList = importRdata(isoformCountMatrix = PRPF4quant$counts,
                         ...
                         isoformExonAnnoation = "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/PRPF4_merged_ISAR/st_merged_test.gtf",
                         isoformNtFasta = "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/PRPF4_merged_ISAR/PRPF4_merged_transcripts.fa",
                         showProgress = TRUE)
...
) #The filtering removed 65360 ( 47.52% of ) transcripts. There is now 73729 isoforms left
...
PRPF4_TPM = PTC_TPM %>% full_join(Novel_TPM) %>% full_join(PTC_MANE_TPM)
write_csv(PRPF4_TPM, "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/Combined_analysis/PRPF4_TPM.csv")

####Comparing the TPMs of different classes of novel transcript####
all_novel = all_switch %>% filter(IF2 <= 0) #976 transcripts
...
all_novel_tpm = all_novel_tpm %>% mutate(kdTPM = rowMeans(select(all_novel_tpm,14:15), na.rm = TRUE)) %>% 
  mutate(wtTPM = rowMeans(select(all_novel_tpm, 15:16), na.rm = TRUE))#976 transcripts
all_novel_tpm_filt = all_novel %>% inner_join(filtTPM, by = "isoform_id")#722 transcripts
all_novel_tpm_filt = all_novel_tpm_filt %>% select(3:12,18:19) %>% mutate(sample = "PRPF4")
write_csv(all_novel_tpm_filt, "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/Combined_analysis/PRPF4_novel_TPM.csv")
...
ggsave("PRPF4_ISAR_lengths.pdf",
       ...
       device = pdf,
       dpi = 300)
write_csv(switching_ORF, "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/Combined_analysis/PRPF4_switch_ORF_length.csv")