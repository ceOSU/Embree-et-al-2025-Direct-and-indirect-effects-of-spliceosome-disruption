...
# Modified on: 8/3/2023
# Analyzing: EIF4A3 shRNA KD from ENCODE
...
#### Copy and Paste ####
#EIF4A3 = Gene of Interest
#C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/ENCODE_EIF4A3_ISAR = C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/ENCODE_EIF4A3_ISAR_ISAR to the working directory
#SRR3469574 = First WT sample
#SRR3469575 = Second WT sample
#SRR3469594   = First KD sample
#SRR3469595 = Second KD sample
...
library(rhdf5)
library(dplyr)
setwd("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/ENCODE_EIF4A3_ISAR")

####Import datasets####
EIF4A3quant = importIsoformExpression(parentDir = "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/ENCODE_EIF4A3_ISAR",
                                      addIsofomIdAsColumn = TRUE, 
                                      showProgress = TRUE,
                                      readLength = 100
) #reads the t_data.ctab file in each sub directory
names(EIF4A3quant)
tail(EIF4A3quant$abundance)
tail(EIF4A3quant$counts)

#Generate the list of isoform switches
myDesign = data.frame(sampleID = c("SRR3469594","SRR3469595","SRR3469574","SRR3469575"),
                      condition = c("KD","KD","WT","WT"))
SwitchList = importRdata(isoformCountMatrix = EIF4A3quant$counts,
                         isoformRepExpression = EIF4A3quant$abundance,
                         designMatrix = myDesign,
                         isoformExonAnnoation = "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/ENCODE_EIF4A3_ISAR/st_merged.gtf",
                         isoformNtFasta = "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/ENCODE_EIF4A3_ISAR/EIF4A3_transcripts.fa",
                         showProgress = TRUE)
...
test = extractSequence(SwitchAndORF,
...
EIF4A3_TPM = PTC_TPM %>% full_join(Novel_TPM) %>% full_join(PTC_MANE_TPM)
write_csv(EIF4A3_TPM, "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/Combined_analysis/EIF4A3_TPM.csv")

####Comparing the TPMs of different classes of novel transcript####
all_novel = all_switch %>% filter(IF2 <= 0) #1053 transcripts
...
all_novel_tpm = all_novel_tpm %>% mutate(kdTPM = rowMeans(select(all_novel_tpm,14:15), na.rm = TRUE)) %>% 
  mutate(wtTPM = rowMeans(select(all_novel_tpm, 15:16), na.rm = TRUE))#1053transcripts
all_novel_tpm_filt = all_novel %>% inner_join(filtTPM, by = "isoform_id")#866 transcripts
all_novel_tpm_filt = all_novel_tpm_filt %>% select(3:12,18:19) %>% mutate(sample = "EIF4A3")
write_csv(all_novel_tpm_filt, "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/Combined_analysis/EIF4A3_novel_TPM.csv")
...
all_switch = extractTopSwitches( #Pulls all switching genes
  ...
  extractGenes = FALSE,
  sortByQvals = FALSE)
count(all_switch,switchConsequencesGene) #FALSE = 2118 TRUE = 1459
ORFanalysis = SwitchAndORF$orfAnalysis
count(ORFanalysis,PTC) #False = 12384 True = 4670 NA = 2364
ORFanalysis = ORFanalysis %>% mutate(Switch = isoform_id %in% all_switch$isoform_id)
count(ORFanalysis,Switch) #False = 15841 True= 3577
switching_ORF = ORFanalysis %>% filter(Switch == TRUE)
count(switching_ORF,PTC) #False =  2358 TRUE = 896 NA =  323
...
length_plot
ggsave("EIF4A3_ISAR_lengths.pdf",
       plot = length_plot,
       height = 10,
       width = 10,
       units = "in",
       device = pdf,
       dpi = 300)
write_csv(switching_ORF, "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/Combined_analysis/EIF4A3_switch_ORF_length.csv")
