...
#### Copy and Paste ####
#UPF1 = Gene of Interest
#C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/ENCODE_UPF1_ISAR = Path to the working directory
#SRR4421434 = First WT sample
#SRR4421435 = Second WT sample
#SRR4421542 = First KD sample
#SRR4421543 = Second KD sample
...
library(rhdf5)
library(dplyr)
setwd("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/ENCODE_UPF1_ISAR")

#Import datasets
UPF1quant = importIsoformExpression(parentDir = "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/ENCODE_UPF1_ISAR",
                                      addIsofomIdAsColumn = TRUE, 
                                      showProgress = TRUE,
                                      readLength = 100
) #reads the t_data.ctab file in each sub directory
names(UPF1quant)
tail(UPF1quant$abundance)
tail(UPF1quant$counts)

#Generate the list of isoform switches
myDesign = data.frame(sampleID = c("SRR4421434","SRR4421435","SRR4421542","SRR4421543"),
                      condition = c("WT","WT","KD","KD"))
SwitchList = importRdata(isoformCountMatrix = UPF1quant$counts,
                         isoformRepExpression = UPF1quant$abundance,
                         designMatrix = myDesign,
                         isoformExonAnnoation = "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/ENCODE_UPF1_ISAR/st_merged.gtf",
                         isoformNtFasta = "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/ENCODE_UPF1_ISAR/UPF1_transcripts.fa",
                         showProgress = TRUE)
...
) #The filtering removed 67184 ( 49.28% of ) transcripts. There is now 74873 isoforms left
...
UPF1_TPM = PTC_TPM %>% full_join(Novel_TPM) %>% full_join(PTC_MANE_TPM)
write_csv(UPF1_TPM, "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/Combined_analysis/UPF1_TPM.csv")

####Comparing the TPMs of different classes of novel transcript####
all_novel = all_switch %>% filter(IF2 <= 0) #446 transcripts
...
all_novel_tpm = all_novel_tpm %>% mutate(kdTPM = rowMeans(select(all_novel_tpm,14:15), na.rm = TRUE)) %>% 
  mutate(wtTPM = rowMeans(select(all_novel_tpm, 15:16), na.rm = TRUE))#446 transcripts
all_novel_tpm_filt = all_novel %>% inner_join(filtTPM, by = "isoform_id")#306 transcripts
all_novel_tpm_filt = all_novel_tpm_filt %>% select(3:12,18:19) %>% mutate(sample = "UPF1")
write_csv(all_novel_tpm_filt, "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/Combined_analysis/UPF1_novel_TPM.csv")
...
ggsave("UPF1_ISAR_lengths.pdf",
       ...
       device = pdf,
       dpi = 300)
write_csv(switching_ORF, "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/Combined_analysis/UPF1_switch_ORF_length.csv")