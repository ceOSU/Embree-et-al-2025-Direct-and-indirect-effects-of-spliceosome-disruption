...
# Date Written: 12/19/2023
# Written by: Caleb Embree
# Modified on: 
# Analyzing:SF3A1 KD from ENCODE
...
#### Copy and Paste ####
#SF3A1 = Gene of Interest
#C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/SF3A1_ISAR = Path to the working directory
#SRR14842960 = First WT sample
#SRR14842961 = Second WT sample
#SRR14842565 = First KD sample
#SRR14842564 = Second KD sample
...
library(tidyverse)
library(biomaRt)
setwd("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/ENCODE_SF3A1_ISAR")

####Import datasets####
SF3A1quant = importIsoformExpression(parentDir = "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/ENCODE_SF3A1_ISAR",
                                   addIsofomIdAsColumn = TRUE, 
                                   showProgress = TRUE,
                                   readLength = 100
) #reads the t_data.ctab file in each sub directory
names(SF3A1quant)
tail(SF3A1quant$abundance)
tail(SF3A1quant$counts)


#Generate the list of isoform switches
myDesign = data.frame(sampleID = c("SRR14842960","SRR14842961","SRR14842565","SRR14842564"),
                      condition = c("WT","WT","KD","KD"))
comparisons = data.frame(condition_1 = "WT",
                         condition_2 = "KD")
SwitchList = importRdata(isoformCountMatrix = SF3A1quant$counts,
                         isoformRepExpression = SF3A1quant$abundance,
                         designMatrix = myDesign,
                         isoformExonAnnoation = "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/ENCODE_SF3A1_ISAR/st_merged.gtf",
                         isoformNtFasta = "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/ENCODE_SF3A1_ISAR/SF3A1_transcripts.fa",
                         showProgress = TRUE,
...
PTC_MANE_TPM = PTC_MANE_TPM %>% dplyr::select(1:11,14) %>%
  mutate(type = "MANE")
SF3A1_TPM = PTC_TPM %>% full_join(Novel_TPM) %>% full_join(PTC_MANE_TPM)
write_csv(SF3A1_TPM, "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/Combined_analysis/SF3A1_TPM.csv")
...
all_novel_tpm_filt = all_novel_tpm_filt %>% dplyr::select(3:12,18:19) %>% mutate(sample = "SF3A1")
write_csv(all_novel_tpm_filt, "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/Combined_analysis/SF3A1_novel_TPM.csv")
...
ggsave("SF3A1_ISAR_lengths.pdf",
       ...
       device = pdf,
       dpi = 300)
write_csv(switching_ORF, "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/Combined_analysis/SF3A1_switch_ORF_length.csv")