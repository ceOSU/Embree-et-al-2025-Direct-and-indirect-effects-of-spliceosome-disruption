...
library(tidyverse)
library(biomaRt)
setwd("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/PRPF31_RP_ISAR")

#Import datasets
PRPF31_RPquant = importIsoformExpression(parentDir = "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/PRPF31_RP_ISAR",
                                      addIsofomIdAsColumn = TRUE, 
                                      showProgress = TRUE,
                                      readLength = 150
) #reads the t_data.ctab file in each sub directory
names(PRPF31_RPquant)
tail(PRPF31_RPquant$abundance)
tail(PRPF31_RPquant$counts)
...
comparisons = data.frame(condition_1 = "WT",
                         condition_2 = "RP")
SwitchList = importRdata(isoformCountMatrix = PRPF31_RPquant$counts,
                         isoformRepExpression = PRPF31_RPquant$abundance,
                         designMatrix = myDesign,
                         isoformExonAnnoation = "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/PRPF31_RP_ISAR/st_merged.gtf",
                         isoformNtFasta = "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/PRPF31_RP_ISAR/PRPF31_RP_iPSC_transcripts.fa",
                         showProgress = TRUE,
...
rawTPM = PRPF31_RPquant$abundance
...
PTC_MANE_TPM = PTC_MANE_TPM %>% dplyr::select(1:9,11:16) %>%
  mutate(type = "MANE")
PRPF31_RP_TPM = PTC_TPM %>% full_join(Novel_TPM) %>% full_join(PTC_MANE_TPM)
write_csv(PRPF31_RP_TPM, "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/Combined_analysis/PRPF31_RP_TPM.csv")
...
all_novel_tpm_filt = all_novel_tpm_filt %>% dplyr::select(3:12,18:19) %>% mutate(sample = "PRPF31_RP")
write_csv(all_novel_tpm_filt, "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/Combined_analysis/PRPF31_RP_novel_TPM.csv")
...
PRPF31_RP_novel_sequence = extractSequence(switchAnalyzeRlist = novel_list,
                                     removeORFwithStop = FALSE,
                                     extractAAseq = FALSE,
                                     removeShortAAseq = FALSE,
                                     removeLongAAseq = FALSE,
                                     outputPrefix = "PRPF31_RP_novel")
novel_annotations = novel_list$isoformFeatures %>% dplyr::select(isoform_id,gene_id,gene_name,PTC)
write_csv(novel_annotations,"PRPF31_RP_novel_iso_features.csv")
...
length_plot
ggsave("PRPF31_RP_ISAR_lengths.pdf",
       plot = length_plot,
       height = 10,
       width = 10,
       units = "in",
       device = pdf,
       dpi = 300)
write_csv(switching_ORF, "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/Combined_analysis/PRPF31_RP_switch_ORF_length.csv")