...
#### Copy and Paste ####
#SNRNP200 = Gene of Interest
#C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/BRR2_K562_ISAR = Path to the working directory
#SRR4421417 = First WT sample
#SRR4421418 = Second WT sample
#SRR4422525 = First KD sample
#SRR4422526 = Second KD sample
...
library(rhdf5)
library(dplyr)
setwd("C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/BRR2_K562_ISAR")

#Import datasets
SNRNP200quant = importIsoformExpression(parentDir = "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/BRR2_K562_ISAR",
                                      addIsofomIdAsColumn = TRUE, 
                                      showProgress = TRUE,
                                      readLength = 100) #reads the t_data.ctab file in each sub directory
names(SNRNP200quant)
tail(SNRNP200quant$abundance)
tail(SNRNP200quant$counts)

#Generate the list of isoform switches
myDesign = data.frame(sampleID = c("SRR4421417","SRR4421418","SRR4422525","SRR4422526"),
                      condition = c("WT","WT","KD","KD"))
SwitchList = importRdata(isoformCountMatrix = SNRNP200quant$counts,
                         isoformRepExpression = SNRNP200quant$abundance,
                         designMatrix = myDesign,
                         isoformExonAnnoation = "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/BRR2_K562_ISAR/st_merged.gtf",
                         isoformNtFasta = "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/BRR2_K562_ISAR/SNRNP200_transcripts.fa",
                         showProgress = TRUE)
...
) #The filtering removed 70035 ( 47.78% of ) transcripts. There is now 76530 isoforms left
...
library(dplyr)
rawTPM = SNRNP200quant$abundance
...
SNRNP200_TPM = PTC_TPM %>% full_join(Novel_TPM) %>% full_join(PTC_MANE_TPM)
write_csv(SNRNP200_TPM, "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/Combined_analysis/SNRNP200_TPM.csv")

####Comparing the TPMs of different classes of novel transcript####
all_novel = all_switch %>% filter(IF2 <= 0) #425 transcripts
all_novel_tpm = all_novel %>% inner_join(rawTPM, by = "isoform_id")
all_novel_tpm = all_novel_tpm %>% mutate(wtTPM = rowMeans(dplyr::select(all_novel_tpm,14:15), na.rm = TRUE)) %>% 
  mutate(kdTPM = rowMeans(dplyr::select(all_novel_tpm, 15:16), na.rm = TRUE))#425 transcripts
all_novel_tpm_filt = all_novel %>% inner_join(filtTPM, by = "isoform_id")#307 transcripts
all_novel_tpm_filt = all_novel_tpm_filt %>% dplyr::select(3:12,18:19) %>% mutate(sample = "SNRNP200")
write_csv(all_novel_tpm_filt, "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/Combined_analysis/SNRNP200_novel_TPM.csv")
...
all_switch = extractTopSwitches( #Pulls all switching genes
  ...
  extractGenes = FALSE,
  sortByQvals = FALSE)
count(all_switch,switchConsequencesGene) #FALSE =  1359 TRUE = 734
ORFanalysis = SwitchAndORF$orfAnalysis
count(ORFanalysis,PTC) #False =  8251 True = 2891 NA = 1619
ORFanalysis = ORFanalysis %>% mutate(Switch = isoform_id %in% all_switch$isoform_id)
count(ORFanalysis,Switch) #False = 10668 True= 2093
switching_ORF = ORFanalysis %>% filter(Switch == TRUE)
count(switching_ORF,PTC) #False = 1422 TRUE = 455 NA =  216
...
length_plot
ggsave("SNRNP200_ISAR_lengths.pdf",
       plot = length_plot,
       height = 10,
       width = 10,
       units = "in",
       device = pdf,
       dpi = 300)
write_csv(switching_ORF, "C:/Users/Caleb/OneDrive - The Ohio State University/BioinfoData/IsoformSwitch/Combined_analysis/SNRNP200_switch_ORF_length.csv")
